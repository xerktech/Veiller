# Veiller Bluetooth SDK: iOS Native API Plan

## Goal

Make the Bluetooth SDK usable directly from bare iOS Swift apps, while Veiller continues to use the same native implementation through a thin Expo adapter.
...
- `VeillerBluetoothSDK.podspec` now defaults to the bare native SDK. Veiller sets `VEILLER_BLUETOOTH_SDK_INCLUDE_EXPO_ADAPTER=1` from its Podfile to include `ExpoModulesCore` and `BluetoothSdkModule.swift`.
...
public final class VeillerBluetoothSDK {
    ...
    public var bluetoothStatus: VeillerBluetoothStatus { get }

    public func startScan(model: VeillerDeviceModel)
    public func stopScan()
    public func connect(to device: VeillerDiscoveredDevice)
    public func connect(model: VeillerDeviceModel, name: String)
    public func connectDefault()
    public func disconnect()
    public func forget()

    public func displayText(_ request: VeillerDisplayTextRequest) async throws
    public func clearDisplay() async throws
    public func showDashboard()

    public func setDashboardPosition(_ request: VeillerDashboardPositionRequest) async throws
    public func setHeadUpAngle(_ angleDegrees: Int) async throws
    public func setScreenDisabled(_ disabled: Bool) async throws
    public func setGalleryModeEnabled(_ enabled: Bool) async throws -> VeillerSettingsAckEvent
    public func setPhotoCaptureDefaults(_ defaults: VeillerPhotoCaptureDefaults) async throws -> VeillerSettingsAckEvent
    public func setButtonVideoRecordingSettings(_ settings: VeillerButtonVideoRecordingSettings) async throws -> VeillerSettingsAckEvent
    public func setButtonCameraLed(enabled: Bool) async throws -> VeillerSettingsAckEvent
    public func setButtonMaxRecordingTime(minutes: Int) async throws -> VeillerSettingsAckEvent
    public func setCameraFov(_ fov: VeillerCameraFov) async throws -> VeillerCameraFovResult

    public func setMicState(_ config: VeillerMicConfiguration)
    public func setPreferredMic(_ preferredMic: VeillerMicPreference)
    public func setOwnAppAudioPlaying(_ playing: Bool)

    public func requestWifiScan() async throws -> [VeillerWifiScanResult]
    public func sendWifiCredentials(ssid: String, password: String) async throws -> VeillerWifiStatusEvent
    public func forgetWifiNetwork(ssid: String) async throws -> VeillerWifiStatusEvent
    public func setHotspotState(enabled: Bool) async throws -> MentraHotspotStatusEvent

    public func requestPhoto(_ request: VeillerPhotoRequest) async throws -> VeillerPhotoResponseEvent
    public func queryGalleryStatus() async throws -> VeillerGalleryStatusEvent
    public func startStream(_ request: VeillerStreamRequest)
    public func stopStream()
    public func startVideoRecording(_ request: VeillerVideoRecordingRequest) async throws -> VeillerVideoRecordingStatusEvent
    public func stopVideoRecording(requestId: String) async throws -> VeillerVideoRecordingStatusEvent
    public func rgbLedControl(_ request: VeillerRgbLedRequest) async throws -> VeillerRgbLedControlResponseEvent

    public func requestVersionInfo() async throws -> VeillerVersionInfoResult
...
The SDK should expose capability state per connected device so customers can disable unsupported UI without relying on no-op behavior. Unsupported operations should either throw a typed `VeillerBluetoothError` or emit a delegate failure, not silently appear to succeed.
...
public protocol VeillerBluetoothSDKDelegate: AnyObject {
    func veillerBluetoothSDK(_ sdk: VeillerBluetoothSDK, didUpdateGlassesStatus status: MentraGlassesStatusUpdate)
    func veillerBluetoothSDK(_ sdk: VeillerBluetoothSDK, didUpdateBluetoothStatus status: VeillerBluetoothStatusUpdate)
    func veillerBluetoothSDK(_ sdk: VeillerBluetoothSDK, didDiscover device: VeillerDiscoveredDevice)
    func veillerBluetoothSDK(_ sdk: VeillerBluetoothSDK, didStopScan reason: VeillerScanStopReason)
    func veillerBluetoothSDK(_ sdk: VeillerBluetoothSDK, didReceive event: VeillerBluetoothEvent)
    ...
    func veillerBluetoothSDK(_ sdk: VeillerBluetoothSDK, didChangeDefaultDevice device: VeillerPairedDevice?)
    func veillerBluetoothSDK(_ sdk: VeillerBluetoothSDK, didLog message: String)
    func veillerBluetoothSDK(_ sdk: VeillerBluetoothSDK, didFail error: VeillerBluetoothError)
}
```
...
```swift
public var events: AsyncStream<VeillerBluetoothEvent> { get }
```

Do not make `AsyncStream` the only integration path in v1. Delegates are still the most familiar iOS library pattern and map cleanly to Veiller's Expo event adapter.

## Public Models

Add typed public models before exposing the facade:

- `VeillerDeviceModel`: `g1`, `g2`, `mentraLive`, `mentraNex`, `mach1`, `z100`, `frame`, `simulated`, `r1`.
- `VeillerDiscoveredDevice`: `model`, `name`, optional `identifier`, optional `rssi`.
...
- `VeillerDisplayTextRequest`, `VeillerDisplayEventRequest`, `VeillerDashboardPositionRequest`, `VeillerDashboardMenuItem`, `VeillerPhotoRequest`, `VeillerStreamRequest`, `VeillerVideoRecordingRequest`, `VeillerMicConfiguration`, `VeillerBluetoothError`.
...
- SDK-owned persistence should use a typed storage/config abstraction or default UserDefaults storage. It should not ask external apps to persist arbitrary Veiller setting keys.
...
public struct VeillerPermissionStatus {
    public let bluetooth: VeillerPermissionState
    public let microphone: VeillerPermissionState
}
```
...
Veiller should keep its TypeScript API stable. `BluetoothSdkModule.swift` becomes an adapter that:

- Owns one `VeillerBluetoothSDK` instance.
- Implements `VeillerBluetoothSDKDelegate`.
...
- Keeps Veiller cloud formatting in TypeScript services such as `SocketComms.ts`, `RestComms.ts`, and `DisplayProcessor.ts`; iOS should emit typed hardware events, not Veiller websocket or REST payloads.

This lets Veiller keep using the SDK while external customers use the native iOS facade.

## Packaging Plan

The bare iOS SDK should be available through CocoaPods first:

- `VeillerBluetoothSDK`: bare native pod with no `ExpoModulesCore` dependency.
- `VeillerBluetoothSDKExpoAdapter` or the existing Expo module target: adapter layer that depends on `VeillerBluetoothSDK` and `ExpoModulesCore`.
...
- `BluetoothSdkModule.swift` and `ExpoModulesCore` are included only when `VEILLER_BLUETOOTH_SDK_INCLUDE_EXPO_ADAPTER=1` is set, which Veiller does from `mobile/ios/Podfile`.
- The Veiller Expo module now owns a `VeillerBluetoothSDK` instance and maps delegate callbacks back to the existing Expo event names. Adapter-only commands such as controller pairing, debug helpers, media volume, RGB LED control, and STT model utilities still call the current internals until the public facade grows those APIs.
...
- Veiller `pod install` and the `VeillerBluetoothSDK` iOS scheme build with the Expo adapter enabled.
...
5. Keep `updateBluetoothSettings`, `"core"` normalization, and `save_setting` as Veiller compatibility plumbing only.
6. Add `mobile/src/services/bluetooth/VeillerBluetoothSdkAdapter.ts` and translate Zustand changes into typed SDK calls setting group by setting group.
7. Move `BluetoothSdkModule.swift` into the adapter layer so the bare SDK pod can remove `ExpoModulesCore`.
8. Update `VeillerBluetoothSDK.podspec` so a clean iOS app can run `pod install` without Expo and without compiling Expo adapter files.
9. Add a bare iOS sample app in `Veiller-Bluetooth-SDK-Partner-Kit` that initializes the SDK, scans, connects, displays text, clears display, applies a core setting, and logs status events.
...
- Veiller still builds and uses the same implementation through `BluetoothSdkModule.swift`.
- Public docs never tell customers to call `DeviceManager`, `DeviceStore`, `Bridge`, `update("bluetooth", ...)`, or handle `save_setting`.
- The iOS sample app in `Veiller-Bluetooth-SDK-Partner-Kit` builds from a clean checkout and exercises the public facade.

# Veiller Bluetooth SDK Plan

## Overview

The **Veiller Bluetooth SDK** is a standalone SDK for communicating with smart glasses. It provides a unified API for Bluetooth communication, audio streaming, display control, and device management.

### Why Build This?

Enterprise customers want to integrate smart glasses into their own mobile apps without using Veiller or our cloud infrastructure. They need a simple SDK that handles the complex BLE protocols, audio codecs, and device quirks - and that's it.

### What Is It?

Most of the device logic already lives in `mobile/modules/core`, but packaging, naming, autolinking, publishing, and Veiller-specific cleanup are still substantial work. The main tasks are:

1. Rename it (`core` → `bluetooth-sdk`)
2. Remove Veiller-specific code (move to `crust` module)
...
- **MentraNex** / **VeillerDisplay** - Protobuf-based display glasses
- **G1** - Display glasses
- **G2** - Display glasses
...
**Veiller** is an operating system and application that _uses_ the Veiller Bluetooth SDK. The target end state for the Bluetooth SDK is a mostly hardware-focused module: BLE, audio, display, camera, device management.

In practice, we have historically thrown some Veiller-specific native helpers into `core` whenever we needed them quickly, especially Android-side app features like notification forwarding. This plan cleans that up by moving obvious app-layer/native Veiller code into `crust`.
...
Everything stays in the existing monorepo structure. We rename `core` to `bluetooth-sdk` and move Veiller-specific code to `crust`.

```
mobile/modules/
├── bluetooth-sdk/           # Renamed from core - publishes as SDK
│   ├── src/                 # TypeScript interface
│   ├── android/             # Android native (com.veiller.bluetoothsdk)
│   └── ios/                 # iOS native (VeillerBluetoothSDK)
│
└── crust/                   # Veiller-specific native code
    ├── src/                 # TypeScript interface
    ├── android/             # Android (com.veiller.crust)
    └── ios/                 # iOS (VeillerCrust)
```
...
- Protobuf support (for MentraNex/VeillerDisplay protocol)

**Native Dependencies Included:**

- LC3 codec (required for audio)
- Protobuf (for MentraNex/VeillerDisplay - naming is legacy but required)
...
Battery/status cleanup is still part of Phase 3. There are a few older native helpers that still format Veiller-specific payloads instead of exposing a clean typed event path.
...
```
offline_mode              # Veiller TypeScript setting for offline behavior
offline_captions_running  # Veiller TypeScript setting tracking offline captions state
```

Do not introduce a new STT control key in this workstream. The offline STT refactor is happening separately.

### Crust (Veiller-Specific)

Everything that's about the Veiller application layer.
...
`auth_email` and `core_token` stay in Bluetooth SDK for now, but they should be treated as legacy Veiller compatibility plumbing rather than as the public Bluetooth SDK authentication model.

These values are not BLE transport credentials. They are Veiller account/cloud values that current MentraLive / ASG companion paths still expect to receive through the device path:

- Veiller TypeScript owns the user/session settings in `useSettingsStore`.
...
- Partner apps that intentionally use Veiller-hosted cloud or ASG companion features may need to provide equivalent credentials through the current compatibility bridge until Phase 6 introduces an explicit native configuration API.
...
- Replace this key/value path with an explicit optional native API, such as `configureCloudCredentials(...)` or a `VeillerBluetoothSdkConfiguration.cloudCredentials` field.
- Make the Veiller-hosted-cloud dependency opt-in and clearly separate from local Bluetooth functionality.
...
- `NotificationListener` - Phone notification forwarding (Veiller feature)
...
These duplicate functions format data for Veiller cloud protocol. They will be DELETED from native code entirely. The TypeScript layer will handle cloud formatting instead (see Phase 3).
...
- `bluetooth-sdk/package.json` - name: `@veiller/bluetooth-sdk`
- `bluetooth-sdk/android/` - package: `com.veiller.bluetoothsdk`
- `bluetooth-sdk/ios/` - module: `VeillerBluetoothSDK`
- `bluetooth-sdk/expo-module.config.json`
- All imports in `mobile/src/` that reference `@veiller/core`
...
```kotlin
// Before: com.veiller.core
// After: com.veiller.bluetoothsdk
...
```swift
// Before: VeillerCore module
// After: VeillerBluetoothSDK module
...
```typescript
// Before
import {CoreModule} from "@veiller/core"
...
## Phase 2: Extract Veiller Code to Crust

### 2.1 Move NotificationListener

**From:** `bluetooth-sdk/android/src/main/java/com/veiller/bluetoothsdk/services/NotificationListener.kt`
**To:** `crust/android/src/main/java/com/veiller/crust/services/NotificationListener.kt`

**Also move from Bluetooth SDK's AndroidManifest.xml to Crust's AndroidManifest.xml:**

```xml
<service android:name="com.veiller.crust.services.NotificationListenerServiceImpl"
...
Crust handles listening to phone notifications and emits Veiller events to the mobile TypeScript layer for cloud reporting. We intentionally are not adding a generic Bluetooth SDK "send notification to display" API in this phase.

### 2.2 Move OS-Specific State

Keep the state split practical in this branch:

- Move obvious Veiller-only native features to Crust, especially notification listening / permission management
...
Remove Veiller-specific side effects from Bluetooth SDK. The `apply()` function should only handle hardware-related side effects:
...
These functions are duplicates that format data for Veiller cloud protocol. They shouldn't exist in native code at all - the TypeScript layer already has the WebSocket connection and should handle all cloud protocol formatting.
...
- Veiller app code formats the cloud `glasses_battery_update` payload in TypeScript
...
3. Handle cloud protocol formatting in Veiller TypeScript layer
...
The Bluetooth SDK React Native/Expo partner surface is the root module exported by `@veiller/bluetooth-sdk`. App code using that adapter should interact with typed async methods and typed event subscriptions, not native `Bridge.kt` / `Bridge.swift` helpers or raw store categories.

Veiller compatibility methods live behind the monorepo-only `@veiller/bluetooth-sdk-internal` alias. That alias resolves to SDK source from the Veiller app config only; it is not exported by the published customer SDK package.
...
**Veiller-internal compatibility helpers:**
...
**Veiller app-layer formatting:**

The SDK emits raw/typed events. Veiller-specific cloud protocol messages stay in the app layer:
...
Internal adapter-only events such as `save_setting`, OTA events, `ws_text`, `ws_bin`, and raw BLE command traces remain available through the Veiller-only `@veiller/bluetooth-sdk-internal` alias while Veiller migrates. New customer-facing features should prefer dedicated typed events.

---

## Native SDK Architecture

The current codebase already has most of the device logic grouped in one module, which is a good starting point for a standalone SDK. The desired end state is now a **bare Android and bare iOS SDK first**, with React Native/Expo treated as a thin adapter used by Veiller and only exposed externally later if needed.

The native SDK entry points should become the stable product API:

- Android: public Kotlin/Java API in `com.veiller.bluetoothsdk`
- iOS: public Swift API in `VeillerBluetoothSDK`
...
```
com.veiller.bluetoothsdk (Android) / VeillerBluetoothSDK (iOS)
├── DeviceManager          # Main orchestrator
├── DeviceStore            # State management
├── Bridge                 # Event emission
├── sgcs/                  # Device implementations
│   ├── MentraLive
│   ├── MentraNex / VeillerDisplay
│   ├── G1 / G2
│   ├── Mach1 / Z100
│   └── Simulated
├── VeillerBluetoothSdk     # Public native SDK facade
...
- Veiller can continue to use the Expo module
...
- Replace template metadata and placeholder repository/package info with Veiller-owned values
- Remove or generalize monorepo-specific assumptions in Expo config plugins, Gradle paths, and Podfile modifications so the package installs cleanly outside Veiller
...
- Android Gradle now has Maven publication metadata for `com.veiller:bluetooth-sdk`.
- CocoaPods metadata is package-driven, includes privacy resources, and no longer relies on monorepo user target paths.
- Expo config plugins no longer mutate host Podfiles with Veiller-specific project names, Firebase pods, or privacy exclusions.
...
- `com.veiller:bluetooth-sdk` - primary Android SDK artifact
- `com.veiller:lc3Lib` - companion LC3 codec artifact required by the SDK
...
android {
    namespace 'com.veiller.bluetoothsdk'
...
**bluetooth-sdk/ios/VeillerBluetoothSDK.podspec:**

```ruby
Pod::Spec.new do |s|
  s.name             = 'VeillerBluetoothSDK'
  ...
  s.resource_bundles = {
    'VeillerBluetoothSDKPrivacy' => ['Source/PrivacyInfo.xcprivacy']
  }
end
```
...
Private repo name: `Veiller-Bluetooth-SDK-Partner-Kit`.
...
These examples live in `Veiller-Bluetooth-SDK-Partner-Kit`, not this monorepo. This repo owns SDK implementation, native artifact publishing, and Veiller adapter/regression tests. If this repo needs local sample projects for CI mechanics, they should mirror the Partner Kit examples rather than becoming the customer-facing source of truth.

React Native / Expo docs can remain a small Veiller adapter note unless/until React Native becomes an external product requirement.
...
- Android: [Veiller Bluetooth SDK Android Native API Plan](./veiller-bluetooth-sdk-android-native-api-plan.md)
- iOS: [Veiller Bluetooth SDK iOS Native API Plan](./veiller-bluetooth-sdk-ios-native-api-plan.md)
...
- Veiller TypeScript uses Zustand stores such as `useSettingsStore`, `useBluetoothStore`, and `useGlassesStore`.
...
- Veiller settings -> native through `BluetoothSdk.updateBluetoothSettings(...)`.
- Native `glasses_status` / `bluetooth_status` -> Veiller Zustand stores.
- Native `save_setting` -> Veiller `useSettingsStore.setSetting(...)`.

That is acceptable as a temporary Veiller compatibility layer, but it should not be the public SDK paradigm. External native customers should use typed commands and typed events, not key/value store categories or Veiller setting names.

Phase 6 should move toward this boundary:

- Keep Zustand as the Veiller app state layer.
...
- Let Veiller have an adapter/service that watches Zustand and calls typed SDK methods.
- Keep `updateBluetoothSettings(...)`, `"core"` category normalization, and `save_setting` only as compatibility plumbing while Veiller migrates.
...
- Android bare artifact: `com.veiller:bluetooth-sdk`, with no Expo Gradle plugin, no `expo-modules-core` project dependency, and no React Native lifecycle assumptions.
- Android Expo adapter: an Expo module / npm package that depends on the bare artifact and forwards events to Veiller.
- iOS bare artifact: `VeillerBluetoothSDK` CocoaPod, with no `ExpoModulesCore` dependency and source globs that exclude Expo module files.
- iOS Expo adapter: a pod/subspec/module that includes `BluetoothSdkModule.swift`, depends on `VeillerBluetoothSDK`, and owns Expo event forwarding.
- React Native / Expo docs stay internal to Veiller unless React Native becomes a customer-facing SDK target again.
...
- Native apps initialize a `VeillerBluetoothSdk` facade with an Android application context and listener.
...
- Create a tiny bare Android sample app in `Veiller-Bluetooth-SDK-Partner-Kit` that uses Maven/local Gradle dependency and the public facade.

### 6.2 Public iOS API

Create a stable Swift facade, as detailed in the iOS native API plan. At this level, the contract is:

- Native apps initialize `VeillerBluetoothSDK`, set a delegate, and optionally consume an async event stream.
...
- Create a tiny bare iOS sample app in `Veiller-Bluetooth-SDK-Partner-Kit` that uses the CocoaPod and public facade.

### 6.3 Veiller Adapter

Veiller should continue to work, but it should use the native SDK through a thin adapter/service:

- `BluetoothSdkModule.kt` calls `VeillerBluetoothSdk`.
- `BluetoothSdkModule.swift` calls `VeillerBluetoothSDK`.
- A Veiller TypeScript service watches relevant Zustand settings and calls typed SDK methods such as `setPreferredMic`, `connectDefault`, and `setMicState`; brightness remains an internal Veiller setting path until the hardware command is reliable.
- Native status callbacks update `useBluetoothStore` / `useGlassesStore`; Veiller TypeScript remains responsible for cloud/websocket formatting.
- The legacy TypeScript API can remain stable during migration while native customers use the native facade.
- Generic store blob syncing should be deprecated once Veiller has moved to typed calls.
- Existing regression tests should continue to pass with little/no changes.

#### 6.3.1 Current Veiller Owners

These are the files to preserve behavior from while extracting the adapter:

- `mobile/src/services/MantleManager.ts` currently owns Bluetooth SDK startup, initial settings sync, Zustand setting subscriptions, native status callbacks, `save_setting`, hardware event subscriptions, and most native-event-to-Veiller routing.
- `mobile/src/stores/settings.ts` owns Veiller setting definitions and the `getBluetoothSdkSettings()` selector that feeds the current native store blob.
- `mobile/src/stores/bluetooth.ts` and `mobile/src/stores/glasses.ts` own Veiller state snapshots that UI and cloud reporting read.
- `mobile/src/services/SocketComms.ts` owns Veiller websocket protocol formatting and cloud-command handling, including VAD, battery, touch, switch, RGB LED, audio frames, stream status, display events, camera/video commands, and mic-state commands.
- `mobile/src/services/RestComms.ts` owns Veiller REST protocol formatting, including glasses status updates, photo responses, phone notifications, calendar/location data, token refresh, and incident/reporting endpoints.
- `mobile/src/services/DisplayProcessor.ts` owns Veiller cloud display payload normalization before payloads reach Bluetooth SDK display APIs.
...
Create the adapter under a Bluetooth-specific services folder so Veiller can migrate direct SDK usage gradually without exposing native store plumbing:

- `mobile/src/services/bluetooth/VeillerBluetoothSdkAdapter.ts`
  - Owns Veiller initialization/cleanup for the Bluetooth SDK.
  - Owns the native facade instance or the Expo adapter module.
  - Provides typed Veiller-facing command wrappers for common hardware operations.
...
- `mobile/src/services/bluetooth/BluetoothEventBridge.ts`
  ...
  - Calls Veiller cloud services where needed, but does not build native SDK payloads.
- `mobile/src/services/bluetooth/VeillerBluetoothSdkAdapter.test.ts`
...
If the implementation stays small, `BluetoothSettingsSync.ts` and `BluetoothEventBridge.ts` can start as private helpers inside `VeillerBluetoothSdkAdapter.ts`; the important boundary is that `MantleManager.ts` stops owning generic native store sync directly.

#### 6.3.3 Migration Order

1. Extract the current `MantleManager` Bluetooth initialization, status listeners, `save_setting`, and settings subscription into `VeillerBluetoothSdkAdapter` without changing behavior.
...
4. Migrate feature-level direct `BluetoothSdk` imports to `VeillerBluetoothSdkAdapter` wrappers when Veiller state, compatibility settings, or cloud side effects are involved. Pure hardware-only feature calls can later call the typed SDK facade directly if that stays clearer.
5. Once no Veiller TypeScript caller needs blob sync, remove `updateBluetoothSettings(...)`, `"core"` category normalization, and `save_setting` from customer-facing docs and eventually from the adapter compatibility layer.

#### 6.3.4 Cloud Formatting Boundary

Phase 6 must not move Veiller cloud protocol formatting back into native SDK code:

- Native Android/iOS Bluetooth SDK emits typed hardware events and accepts typed hardware commands only.
- `SocketComms.ts` remains the owner for websocket JSON sent to Veiller cloud.
- `RestComms.ts` remains the owner for REST payloads sent to Veiller cloud.
- `DisplayProcessor.ts` remains the owner for Veiller display payload normalization before display commands reach the SDK.
- `VeillerBluetoothSdkAdapter` may route events to those services, but it should not introduce native-formatted Veiller cloud payloads.
- Partner Kit/native SDK docs should describe typed hardware events, not Veiller websocket message formats.

### 6.4 Native API Documentation

Update `Veiller-Bluetooth-SDK-Partner-Kit` to lead with:
...
- React Native/Expo adapter note for Veiller only
...
- Android: publish `com.veiller:bluetooth-sdk` and `com.veiller:lc3Lib` to Maven local, then run the Partner Kit Android example `assembleDebug`.
- iOS: point the Partner Kit Podfile at the local `VeillerBluetoothSDK` podspec, run `pod install`, then run `xcodebuild` for the iOS example.

### 6.5 Validation

Phase 6 is not complete until:

- Bare Android sample app in `Veiller-Bluetooth-SDK-Partner-Kit` builds and can initialize the SDK.
- Bare iOS sample app in `Veiller-Bluetooth-SDK-Partner-Kit` runs `pod install`, builds, and can initialize the SDK.
- Veiller still builds and uses the SDK through the Expo adapter.
...
- TypeScript adapter tests should cover Veiller store-to-SDK sync, including initial setting sync, setting diffs, ignored non-SDK settings, typed status callbacks, and cloud-formatting ownership in TypeScript.
...
- Native package integration should validate fresh external installs: Maven local publication for Android, CocoaPods install for iOS, and Veiller Expo adapter autolinking.
- Crust integration tests should cover Veiller-only phone notification listening/permission behavior separately from Bluetooth SDK hardware tests.
...
- Cloud ownership: typed hardware events should be formatted for Veiller websocket/rest protocols in TypeScript, not in the native Bluetooth SDK.

### CI Gates

- Refactor PRs should run the touched focused suites with `cd mobile && bun run test -- <paths> --runInBand`, plus `git diff --check`.
- Native Android changes should run the relevant Gradle compile/test tasks and, for publishing changes, local Maven publication for both `com.veiller:bluetooth-sdk` and `com.veiller:lc3Lib`.
- Native iOS changes should validate podspec syntax / CocoaPods install behavior and a Veiller iOS build when the bridge, podspec, or native Swift/Obj-C files change.
...
- Publishing/release PRs should additionally test a fresh bare Android app and bare iOS app consuming the SDK artifact, not only the monorepo Veiller app.

### Device And OS Matrix

- Always keep the simulated device path useful for CI and local smoke tests.
- Hardware validation before release should include MentraLive plus at least one display-only family such as MentraNex/VeillerDisplay, G1/G2, Mach1, or Vuzix Z100.
...
- Record device model, OS version, firmware version, and whether the run used Veiller cloud features or local-only SDK features.
...
- [x] Update package.json name to `@veiller/bluetooth-sdk`
- [x] Update Android package: `com.veiller.core` -> `com.veiller.bluetoothsdk`
- [x] Update iOS module name
- [x] Update Veiller imports to consume `@veiller/bluetooth-sdk`
...
- [x] Keep the state split practical instead of redesigning it:
  - [x] Move obvious Veiller-only native features to Crust
...
- [x] Wire up Crust to Veiller app layer
- [x] Verify Veiller still works on the covered Android/iOS validation paths
...
- [ ] Add `mobile/src/services/bluetooth/VeillerBluetoothSdkAdapter.ts`
...
- [x] Keep Veiller Expo adapter working on top of native facades
- [x] Create bare Android sample app in `Veiller-Bluetooth-SDK-Partner-Kit`
- [x] Create bare iOS sample app in `Veiller-Bluetooth-SDK-Partner-Kit`
...
- [x] Validate Veiller mobile app still passes relevant regression tests

Current Phase 6 validation status:

- Android `:veiller-bluetooth-sdk:compileReleaseKotlin` passes with the new native facade and event/store fanout.
- Android `:veiller-bluetooth-sdk:compileDebugKotlin` passes with the Expo module now owning `VeillerBluetoothSdk` for shared commands and event forwarding.
...
- Veiller Android `:app:assembleDebug` passes with the cleaned Android publication/manifest setup.
- iOS `VeillerBluetoothSDK` scheme builds for iOS simulator with the new Swift facade and event/store fanout.
- iOS `BluetoothSdkModule.swift` now owns a `VeillerBluetoothSDK` instance, forwards delegate callbacks to the existing Expo event names, and keeps adapter-only calls direct until those APIs are added to the public facade.
- Local Maven publication succeeds for `com.veiller:bluetooth-sdk` and `com.veiller:lc3Lib`; the Bluetooth SDK POM resolves `lc3Lib` as `com.veiller:lc3Lib:0.1.0`, no longer exposes `host.exp.exponent:expo-modules-core`, and the LC3 POM no longer publishes legacy app/cloud dependencies.
...
- The bare iOS podspec defaults to the native SDK only: no `ExpoModulesCore` dependency and no `BluetoothSdkModule.swift` adapter source unless `VEILLER_BLUETOOTH_SDK_INCLUDE_EXPO_ADAPTER=1` is set.
- Veiller opts into the Expo adapter from `mobile/ios/Podfile`, preserving the current Expo module path while bare CocoaPods consumers get the native SDK surface.
...
### Phase 7: Veiller Migration

When we delete the duplicate cloud-formatting functions from Bridge, the Veiller TypeScript layer needs to handle that formatting instead.
...
Some other helpers (battery / VAD / video / ws_text passthroughs) still format Veiller cloud payloads in native and should move to TypeScript.
```

**New flow:**

```
Native: hardware emits typed/raw device events only → TypeScript formats any Veiller cloud payloads that are still needed → TypeScript sends to WebSocket
```

**Files to update in Veiller TypeScript:**
...
- [ ] Announce release
- [ ] Update Veiller to use published package (optional - can continue using monorepo)
...
1. **Published Packages**
   - `@veiller/bluetooth-sdk` on npm
   - `com.veiller:bluetooth-sdk` on Maven Central
   - `VeillerBluetoothSDK` on CocoaPods

2. **Clean Separation**
   - Bluetooth SDK has no Veiller-specific code
...
4. **Veiller Compatibility**
   - Veiller app continues to work

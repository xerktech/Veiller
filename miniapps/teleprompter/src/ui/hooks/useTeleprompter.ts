...
export function useTeleprompter() {
  ...
  useEffect(() => {
    ...
    veiller.send("tp:request-snapshot", {})
  ...
  const setScript = useCallback(
    (script: string) => {
      ...
      veiller.send("tp:set-script", {script})
    },
    [patchSettings],
  )
  ...
  const setLines = useCallback(
    (lines: number) => {
      ...
      veiller.send("tp:set-lines", {lines})
    },
    [patchSettings],
  )
  const setWidth = useCallback(
    (width: LineWidth) => {
      ...
      veiller.send("tp:set-width", {width})
    },
    [patchSettings],
  )
  const setVoiceFollow = useCallback(
    (enabled: boolean) => {
      ...
      veiller.send("tp:set-voice-follow", {enabled})
    },
    [patchSettings],
  )
  const setAutoRestart = useCallback(
    (enabled: boolean) => {
      ...
      veiller.send("tp:set-auto-restart", {enabled})
    },
    [patchSettings],
  )
  const setShowTimecode = useCallback(
    (enabled: boolean) => {
      ...
      veiller.send("tp:set-show-timecode", {enabled})
    },
    [patchSettings],
  )

  // ── Transport ──────────────────────────────────────────────────────────
  const play = useCallback(() => veiller.send("tp:play", {}), [])
  const pause = useCallback(() => veiller.send("tp:pause", {}), [])
  const restart = useCallback(() => veiller.send("tp:restart", {}), [])
  const seek = useCallback((percent: number) => veiller.send("tp:seek", {percent}), [])
  const nudge = useCallback((lines: number) => veiller.send("tp:nudge", {lines}), [])

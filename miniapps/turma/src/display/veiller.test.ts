// Tests for the new Veiller scene backend (Veiller port — no upstream
...
import { VeillerDisplay, type VeillerDisplaySession, type RenderElementLike, type TouchDataLike } from "./veiller.ts";
import type { ScreenModel } from "../core/render.ts";
import type { InputEvent } from "../core/types.ts";

class FakeSession implements VeillerDisplaySession {
...
describe("VeillerDisplay", () => {
  let session: FakeSession;
  let display: VeillerDisplay;

  beforeEach(async () => {
    fakeTimers.useFakeTimers();
    fakeTimers.setSystemTime(1_000_000);
    session = new FakeSession();
    display = new VeillerDisplay(session);
    await display.start();
  });

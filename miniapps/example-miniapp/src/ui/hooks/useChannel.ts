 ...
 *
 * The WebView's `veiller` global doesn't expose state; it's a
...
export function useChannel<C extends keyof Channels & string>(
  ...
  useEffect(() => {
    ...
    // `veiller.on` rejects RPC channels at the type level. This hook is
    ...
    return (veiller.on as (c: string, cb: (p: unknown) => void) => () => void)(
      channel,

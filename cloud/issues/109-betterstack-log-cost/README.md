...
1. **Intended path:** app writes Pino JSON to stdout -> our Vector DaemonSet (Helm, `cloud/infra/betterstack-logs/values.yaml`) filters to cloud containers, flattens, ships to the "VeillerCloud - Prod" source. This is the source used for Live Tail / dashboard search.
...
The collector log copies had no consumers: the bstack CLI, both dashboards, and the runbooks only use the collectors' *metrics* tables and the VeillerCloud - Prod *logs* table.
...
Decisions: keep the VeillerCloud - Prod source fully intact (info level included) because it is actively used for reading/searching logs in the dashboard; kill only duplicates and retention.
...
3. Retention cuts to the lowest tier the API allows (30 days; allowed values are 30/60/90/180/365+):
   - VeillerCloud - Prod: logs and metrics -> 30d
...
- **Live-verified 2026-07-14 22:29 UTC:** collector log ingest flatlined to zero across the active regions roughly 45 minutes after the PATCH (managed agents pick the config up on their poll cycle, no DaemonSet restart needed). The intended VeillerCloud - Prod path still flowing normally.

/**
 * VeillerClient - Main client class for connecting to Veiller cloud
 *
 * This is a pure TypeScript SDK that provides a clean, production-ready interface
 * for interacting with the Veiller cloud platform. It mirrors the cloud architecture
...
/**
 * Main client class providing clean public API for Veiller cloud interaction
 */
export class VeillerClient extends EventEmitter {
  ...
  /**
   * Connect to the Veiller cloud server
   */
  async connect(serverUrl?: string): Promise<void> {
    ...
    try {
      ...
      if (this.config.debug?.logLevel === "debug") {
        console.log(`[VeillerClient] Connected to ${url}`);
      }
    } catch (error) {
  ...
  async disconnect(): Promise<void> {
    ...
    try {
      ...
      if (this.config.debug?.logLevel === "debug") {
        console.log("[VeillerClient] Disconnected");
      }
    } catch (error) {
  ...
  async startApp(packageName: string): Promise<void> {
    ...
    if (this.appManager.isAppRunning(packageName)) {
      console.log(`[VeillerClient] App ${packageName} is already running`);
      return;
    }
  ...
  async stopApp(packageName: string): Promise<void> {
    ...
    if (!this.appManager.isAppRunning(packageName)) {
      console.log(`[VeillerClient] App ${packageName} is already stopped`);
      return;
    }

# Veiller React Auth Library (`@veiller/react`)

This library simplifies authentication for React-based webviews running within the Veiller manager application. It handles the extraction and verification of the `aos_signed_user_token` provided by the Veiller system and makes user information available through a React Context and Hook.

Check out the full [Veiller React Documentation](https://docs.mentra.glass/react-webviews) for more details.

## Prerequisites

-   React 16.8+
-   Your webview must be opened by the Veiller manager application, which will append the `aos_signed_user_token` to the URL.

## Installation

```bash
bun add @veiller/react
# or
npm install @veiller/react
# or
yarn add @veiller/react
```

## Usage

### 1. Wrap your application with `VeillerAuthProvider`
...
### 2. Access authentication state using `UseVeillerAuth`
...
const MyComponent = () => {
  const { userId, frontendToken, isLoading, error, isAuthenticated, logout } = UseVeillerAuth();
  ...
  if (error) {
    return (
      <div>
        ...
        <p>Please ensure you are opening this page from the Veiller app.</p>
      </div>
    );
  }

  if (!isAuthenticated || !userId) {
    return <p>Not authenticated. Please open from the Veiller manager app.</p>;
  }

  // You are authenticated!
  return (
    <div>
      <h1>Welcome, Veiller User!</h1>
      <p>User ID: {userId}</p>
...
1.  When your webview is loaded by the Veiller manager, it appends an `aos_signed_user_token` (a JWT) as a URL query parameter.
2.  The `VeillerAuthProvider` attempts to find this token.
3.  It verifies the token's signature against the Veiller Cloud public key and checks its claims (like issuer and expiration).
4.  If valid, it extracts the `userId` (from the `sub` claim) and a `frontendToken` (another JWT from the payload).
5.  These `userId` and `frontendToken` are then stored in `localStorage` and made available via the `UseVeillerAuth` hook.
...
The `frontendToken` obtained from `UseVeillerAuth` is a JWT. You should send this token in the `Authorization` header as a Bearer token when making requests from your webview to **your App's backend API**.  The Veiller SDK will automatically verify this token.

```typescript
// Example of an authenticated API call
const { frontendToken } = UseVeillerAuth();
...
This library includes full TypeScript support. The `UseVeillerAuth` hook returns a typed object with the following interface:
...
*   **"Token validation failed" / "Token expired":**
    *   Ensure the clock on the device running the Veiller manager is synchronized.
    ...
    *   The Veiller manager app should be providing a fresh token.
*   **"Veiller signed user token not found":**
    *   Make sure your webview is being launched from the Veiller manager app.

...
export interface VeillerSessionConfig extends LoggerConfig {
  packageName: string;
  apiKey: string;
  sessionId: string;
  transport: Transport;
...
export class VeillerSession {
  ...
  constructor(config: VeillerSessionConfig) {
    ...
    this._lifecycleManager = new _ConnectionManager({
      ...
      onError: (error) => {
        ...
        this.logger.error(error, "VeillerSession transport error");
      },
    });
  ...
  private registerCoreHandlers(): void {
    ...
    this.cleanupTasks.push(
      this._router.messageHandlers.register(CloudToAppMessageType.CONNECTION_ERROR, (message) => {
        this.emit("error", new Error(message.message ?? "VeillerSession connection error"));
      }),
    );
  ...
  private applyVeillerSettings(settings: Record<string, any>): void {
    ...
    if (typeof timezone === "string" && timezone.length > 0) {
      try {
        ...
        this.logger.warn({ timezone, error }, "VeillerSession received invalid timezone");
      }
    }
  }

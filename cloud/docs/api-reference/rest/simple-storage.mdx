...
The Simple Storage API provides a localStorage-like interface for Veiller Apps to store and retrieve key-value data. Data is isolated by user email (`:email`) and the authenticated `packageName` from the Authorization header.

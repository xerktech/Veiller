...
const args: HarnessArgs = {
  ...
  label: rawArgs.get("label") ?? "veiller-path-storm",
  packages: (rawArgs.get("packages")?.split(",").filter(Boolean) ?? DEFAULT_PACKAGES).map((packageName) =>
    packageName === "com.veiller.captions" ? "com.veiller.captions.debug" : packageName,
  ),
};
...
console.log(
  JSON.stringify({
    event: "veiller-harness-complete",
    label: args.label,
...
function defaultSubscriptions(packageName: string): string[] {
  if (packageName === "com.veiller.captions") {

# VeillerJS — Two-Layer Local Miniapp Architecture
...
- **2026-05 navigation + heading SDK** (PR #2779, merged into
  `veiller-miniapp-sdk-2`). Adds `session.navigation`,
...
`WKWebView` in the Veiller Manager iOS app. Stress test on iPhone 15
release build:
...
and the Veiller app backgrounded, with glasses connected**. On
...
1. **Background layer (VeillerJS)** — a per-miniapp JS context
   ...
   of platform.
2. **UI layer (VeillerUI)** — a `WKWebView` (iOS) / Android
...
```
┌─────────────────────────────────────────────────────────────────┐
│                  Veiller Manager (host RN app)                    │
│                                                                  │
│   ┌──────────────────────┐        ┌─────────────────────────┐    │
│   │ Native iOS / Android │ ←────→ │ VeillerJS native router  │    │
...
│   └──────────┬──────────┘         └─────────────────────┘        │
│              │                                                   │
│              │ ui bus (veiller.send / veiller.on)                  │
│              ▼                                                   │
│   ┌─────────────────────┐                                        │
│   │ WebView (transient) │ ← only when user is looking            │
│   │  window.veiller      │   at this miniapp's settings           │
│   │  no native access   │                                        │
│   └─────────────────────┘                                        │
└──────────────────────────────────────────────────────────────────┘
```
...
- Background ↔ WebView: typed `veiller.send`/`veiller.on` message bus
...
```
mobile/modules/crust/android/src/main/java/com/veiller/crust/
...
'{"VEILLER_RUN_JSC_BENCH":"1"}' com.veiller.veiller` then
`xcrun devicectl device copy from --domain-type appDataContainer
--domain-identifier com.veiller.veiller --source Documents/jsc-spike.log`.
...
├── android/
│   └── src/main/java/com/veiller/crust/
...
(`mobile/modules/veillerjs-runtime/`) so JS shims iterate independently
...
// Event "veillerjs_message" — fires when a JSContext calls __dispatch
// Delivered via Expo `sendEvent("veillerjs_message", ...)` from native;
// JS subscribes via `Crust.addListener("veillerjs_message", handler)`.
```
...
1. **Host RN runtime** (Hermes, the React Native bridge — the
   "main" JS the Veiller app runs in). This is where
   `LocalMiniappRuntime` / `VeillerJSRouter` lives, and where the
...
miniapp BG JS calls session.display.showTextWall("hi")
  ↓ SDK shim (in @veiller/miniapp/background)
  ↓
...
Tag the call with this JSContext's packageName
  ↓
Send Expo Module event "veillerjs_message" with payload
  { packageName, iface: "display", method: "showTextWall", args }
  ↓ via Expo Module API `sendEvent("veillerjs_message", payload)`
    (same call shape on iOS and Android — Expo Modules normalize
    the underlying NativeEventEmitter / DeviceEventEmitter delivery)
  ↓
React Native receives in VeillerJSRouter (host RN runtime),
  subscribed via Crust.addListener("veillerjs_message", ...)
  ↓
...
```
VeillerJSRouter (host RN) computes the event payload
  ↓
...
```
WebView event handler calls veiller.send("show-glasses", { text })
  ↓ injected window.veiller shim
  ↓
...
Cloud server sends "phone_photo_ready" / "phone_stream_status" /
  ...
  ↓
VeillerJSRouter.handleCloudMessage(msg) (lifted from
...
2. **All four flows use Expo Module events / function calls as the
   ...
   same downstream path once it's inside `VeillerJSRouter`). No
   custom IPC. Same plumbing we use for every other native module.
3. **Cloud responses are a third inbound source** for `VeillerJSRouter`,
   alongside JSContext-originating `__dispatch` events and WebView
   `veiller.send`. Don't forget this when wiring the router; it's
...
1. **iOS:** Create `JSVirtualMachine` + `JSContext` on a dedicated thread.
   **Android:** Create a `Zipline` instance (which owns a `QuickJs`
   ...
   isolation.
2. **iOS:** Set `context.name = "VeillerJS: <id>"` and (DEBUG-only)
...
6. Inject the SDK shim (`@veiller/miniapp/background` typed wrappers around
...
### Bridge 1: VeillerJS ↔ Native (full power)
...
WebView side reads `veiller.on("pivot", ...)`. Same rule as every
...
interface UIModule {
  ...
  // Fires once each time a WebView mounts and acks `veiller.ready()`.
...
Asymmetry with `veiller.send` (the WebView side): WebView-side
...
- One-shots (`session.sendOneShot`) return immediately with `null`.
  ...
  `veillerjs_message` event with `iface: "_error"`.
- Requests (`session.sendRequest`) return a `Promise` resolved when
  native posts the matching `veillerjs_message` event with the same
...
### Bridge 2: WebView ↔ VeillerJS (per-miniapp message bus)

Auto-injected into each WebView at mount time:

```typescript
declare const veiller: {
  send<C extends keyof Channels>(channel: C, payload: Channels[C]): void
...
`veiller.send()` does NOT go to native. It goes to the bound
...
Host → WebView (via `webview.injectJavaScript("window.__veiller.recv(...)")`):
```typescript
type UIOutbound =
  | { type: "msg"; seq: number; channel: string; payload: unknown }
...
**Synthetic channels:** `veiller.on("__open__", ...)` and
`veiller.on("__close__", ...)` are reserved for lifecycle events and
...
```
WebView event
  → veiller.send(channel, payload)         [WebView side]
  → host router                            [native]
...
├── tsconfig.json
├── src/
│   ├── background/            # VeillerJS entrypoint — always running
...
`src/ui/index.tsx` — uses React helpers from `@veiller/miniapp/ui`,
...
veiller.on("state", ({notes: incoming}) => {
  notes = incoming
  render()
})

veiller.on("note-added", ({note}) => {
  notes.push(note)
  render()
})
...
| `mobile/src/services/MantleManager.ts` | ~1,060 | Runtime-hook adapter that wires `NavigationService`/`HeadingService`/`setLocationTier` into the island runtime. Adapter shape carries forward — point it at the new `VeillerJSRouter` instead of `LocalMiniappRuntime`. |
...
| `sdk/create-veiller-miniapp/bin/index.ts` + template | ~150 + template | Scaffolder logic survives (clack prompts, validation, template substitution). **Template files rewrite:** scaffold `src/background/index.ts`, `src/ui/`, `src/shared/channels.ts` instead of single React SPA. |
...
| `mobile/modules/engine/src/services/WebviewBridge.ts` | 50 | Replaced by two sibling routers: `VeillerJSRouter` (JSContext fan-out) + `VeillerUIRouter` (settings WebView ↔ bound JSContext). |
| `mobile/modules/miniapp/src/globals.ts` | 62 | `window.Veiller` is WebView-presentational. Keep file for WebView; JSContext gets a different injected globals object. |
| `mobile/modules/miniapp/src/index.ts` | 108 | Replaced by two sub-path entry points via `package.json` `exports`: `@veiller/miniapp/background` (session API for the JSContext layer) and `@veiller/miniapp/ui` (WebView-side `veiller` global + React hooks). No bare `@veiller/miniapp` import — sub-paths only. |
...
- **iOS bootstrap:** `injectedJavaScriptBeforeContentLoaded` is
  reliable on iOS — use it to install the `window.veiller` shim before
  page JS runs.
- **Android bootstrap:** `injectedJavaScriptBeforeContentLoaded` is
  ...
  installs the runtime `window.veiller` shim from a small inline
...
- **`VeillerUIRouter`** — when WebView mounts, host binds it to a
  JSContext and routes `veiller.send`/`veiller.on` between them via
...
- **`VeillerJSRouter.ts`** — host-side router that subscribes to
  `Crust.addListener("veillerjs_message", ...)`, looks up the
...
- **`VeillerUIRouter.ts`** — bridges the bound WebView to its
  JSContext sibling, routing `veiller.send/on` between them.
...
- **`window.veiller` shim** — typed `send`/`on`/`ready` injected into
...
Polyfill bundle (in new `mobile/modules/veillerjs-runtime/`):
...
**Cloud:** none for V1. The CLI's `bun veiller-miniapp release`
...
| `localStorage` | Bridge | — | ~50 over `NSUserDefaults` (iOS) / `SharedPreferences` (Android) with `"VeillerJS-{packageName}"` suite/file | |
...
1. User scans `veiller-miniapp release` QR or hits the dev URL. Host
...
3. Host injects the `window.veiller` shim. **iOS:** via
   ...
   **Android:** via `injectedJavaScriptObject` + a small inline
   ...
   `window.__veiller.recv(...)` calls injected by the host using
...
6. WebView mounts. `index.tsx` runs. Calls `veiller.ready()`.
...
- **WebViews cannot make BLE calls.** No native API in WebView.
  Only `veiller.send('show-text', {...})` to background.
...
NSUserDefaults under suite name `VeillerJS-<packageName>` (per the
...
to `Application Support/veiller/miniapps/` so it isn't user-visible
...
1. **Discover.** User scans QR from `veiller-miniapp release` or
   types the dev URL.
...
1. **`veiller-miniapp dev`** — hot-reload over LAN. Bundle never lands
   on disk; runs from in-memory dev server.
2. **`veiller-miniapp release`** — produces a zip, serves over LAN,
...
`veiller-miniapp release` and the user re-scans QR. New version
...
4. If user opted to delete data: delete the app's `session.storage`
   namespace (NSUserDefaults `VeillerJS-<packageName>` suite, or
   the equivalent SharedPreferences file on Android).
5. Revoke permissions; remove from local cache.

### Host app upgrade

When the Veiller Manager app updates (App Store / Play Store
...
ONE Sentry project (`veiller-mobile`). `release = host_version`.
...
- **Memory growth:** sample every 30s. Linear-regression last 20
  ...
  `veiller.runtime.leak_suspected` event with packageName.
...
JSContext; show "Disabled by Veiller" tile. Will be required for
...
- Add Expo Functions to `CrustModule.swift`: `veillerJsSpawn`,
  `veillerJsEvaluate`, `veillerJsKill`, `veillerJsDispatchToJs`. Event
  `veillerjs_message`. Verify the new module registers cleanly via
...
- New Expo module: `mobile/modules/veillerjs-runtime/`. Needs
...
- Write thin bridges in `JSCPolyfillBridge.swift` for setTimeout
  ...
  `VeillerJS-{packageName}` suite), crypto (CryptoKit).
...
- New Kotlin files in `mobile/modules/crust/android/src/main/java/com/veiller/crust/`:
...
- Bridge concretes: OkHttp (fetch/WebSocket), `Handler.postDelayed`
  ...
  (`localStorage` with `VeillerJS-{packageName}` name), `javax.crypto`
...
### Phase 2 — Refactor `LocalMiniappRuntime` → `VeillerJSRouter` (~3 weeks)
...
- Move all 33 dispatch arms in `handleRawMessage`'s switch to a new
  `VeillerJSRouter` class taking `(packageName, payload)` from
...
- **Carry forward the navigation accounting too.** The handler set
  ...
  belongs on `VeillerJSRouter` — verbatim except for the front-door
  swap.
...
`VeillerJSRouter` is platform-agnostic; it talks to whichever
...
- Unit: `VeillerJSRouter` routes `__dispatch` to the correct handler
...
- **Bootstrap-before-page-load is the only thing that differs by
  ...
  the `window.veiller` shim — the
  `injectedJavaScriptBeforeContentLoaded` prop is documented
  unreliable on Android (react-native-webview docs warn explicitly,
  refs issues #1099 / #1609).
- We layer the new `veiller.send/on/ready` API on top of this
  ...
  a typed envelope.
- Build the new `VeillerUIRouter` over `react-native-webview`'s
...
- New `VeillerUIRouter` (in `mobile/modules/engine/src/services/`,
  ...
  outputs back via `injectJavaScript("window.__veiller.recv(...)")`.
- `window.veiller` shim (~50 LoC). Full surface:
...
- `dev-reload.ts` (60 LoC) — needs an update so the new shim's
  `window.__veiller.recv()` direct call still triggers reload events.
...
- Unit: `VeillerUIRouter` routes `veiller.send` → JSContext's
...
V1 ships LAN-only sideloading via `bun veiller-miniapp dev` and
`bun veiller-miniapp release`. **No store, no signing, no remote
...
- **Build-time env vars for miniapps.** Adopt the same
  `VEILLER_PUBLIC_*` (or similar) prefix convention the host uses
  ...
  backend, not in `VEILLER_PUBLIC_*`. The host's `veiller-miniapp dev`
...
- `Documents/lmas/` → `Application Support/veiller/` migration.
...
- **Package naming: sub-paths under `@veiller/miniapp` (decided).**
  - `@veiller/miniapp/background` — session API (`glasses`, `phone`,
    `input`, `display`, `transcription`, `mic`, `speaker`, `camera`,
    `dashboard`, `led`, `location`, `imu`, `permissions`, `storage`,
    `stream`, `system`, `ui`, `diagnostics`).
  - `@veiller/miniapp/ui` — WebView-side `veiller` global, React
    hooks, `VeillerProvider`, settings-page components.
  - **No bare `@veiller/miniapp` import.** Sub-paths only. There
    is no installed-base of miniapps to maintain back-compat with —
    we have one example app and rewrite it.
  - The unrelated cloud-side `@veiller/sdk` package keeps its name —
  ...
  - Set up via `package.json` `exports` field with separate
    ...
    different ambient types per layer (e.g. `veiller: ...` global
...
- Split `mobile/modules/miniapp/src/index.ts` (108 LoC) into
  ...
  rename.
- Update `sdk/create-veiller-miniapp/template/`: today scaffolds a
  ...
  Scaffolder logic at `sdk/create-veiller-miniapp/bin/index.ts`
  (149 LoC) survives — only template files change.
- Restructure `sdk/example-miniapp/` per **Appendix A** (entire
  React SPA migration). The new `VeillerProvider` and `useSession`
  implementations live in `@veiller/miniapp/ui` and wrap the message
...
**Android within Phase 5.** None — `@veiller/miniapp/{background,ui}`
is platform-agnostic JS. Same package serves both platforms.

**Acceptance gate (how to know Phase 5 is done):**
- `bun create veiller-miniapp test-app` produces a working scaffold
  with the symmetric `src/background/` + `src/ui/` layout from
  Appendix A.
- TypeScript compile passes for both `@veiller/miniapp/background`
  and `@veiller/miniapp/ui` sub-paths with their separate ambient
  types. **Test harness:** new file
  `sdk/example-miniapp/test/sub-path-types.test-d.ts` (or in
  `@veiller/miniapp` itself) using `tsd` or `@ts-expect-error` blocks:
  one block imports `from "@veiller/miniapp/background"` and asserts
  `// @ts-expect-error` on a reference to the `veiller` global; a
  second block imports `from "@veiller/miniapp/ui"` and asserts
  `veiller.send` typechecks. `bun typecheck` must pass.
...
V1 has **zero cloud work**. The CLI's `bun veiller-miniapp release`
...
location-rate aggregation. Phase 2's `VeillerJSRouter` rewrite
...
3. The 919-LoC `NavigationPage.tsx` swaps every `user.navigation.*`
   call for `veiller.send("trip:*", ...)`. The page becomes a pure
...
"the SDK split is real" — if `bun veiller-miniapp dev` from the new
...
sent, hydrated on `veiller.ready()` from a one-shot
...
entry calls `veiller.ready()` and subscribes to `veiller.on(...)` for
...
- **(a) Mostly read-only testers** (Permissions, Storage,
  ...
  `veiller.send("tester:start", {iface: "imu"})`; background opens
  the relevant subscription, pipes events back via
  `veiller.send("tester:event", {iface, payload})`. Page renders
  what it sees. ~30 LoC delta per tester.
- **(b) Fire-and-forget testers** (Display, Led, Speaker, Phone):
  page calls `veiller.send("tester:fire", {iface, method, args})`;
...
1. `bun create veiller-miniapp my-app` produces a scaffold matching
   the target structure above.
2. `bun veiller-miniapp dev` in `sdk/example-miniapp/` builds both
...
- **`sdk/create-veiller-miniapp/bin/index.ts` + template** —
...
- **`sdk/miniapp-cli/`** — see Phase 4 file list. `dev.ts` evolves to
  ...
  WebSocket. (Phase 4.)
- **`@veiller/miniapp` package.json `exports`** — gain
  `./background` and `./ui` sub-paths; the bare `@veiller/miniapp`
...
1. **WebView opens, fires events before background is ready.**
   `veiller.ready()` is required. SDK buffers `veiller.send()` until
...
3. **User opens UI, WebView loads, but background is mid-async-init.**
   SDK's `veiller.ready()` retries with exponential backoff until
...
7. **Uninstall-while-WebView-foregrounded.** User uninstalls a
   ...
   bundle. In-flight `veiller.send` from a WebView that's gone
...
6. **Should the SDK provide a typed RPC helper over the UI bus?**
   The raw bus today is `veiller.send(channel, payload)` +
...
- `bun create veiller-miniapp` to a working miniapp in under 30

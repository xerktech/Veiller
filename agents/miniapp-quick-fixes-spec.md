...
This spec bundles four small, mostly-independent dev-ex improvements identified in the post-PR-#2512 feedback round. They're grouped because each one is small enough not to need its own spec, and they share the same target audience (miniapp authors using `veiller-miniapp dev`).
...
## 1. Live reload for `veiller-miniapp dev`

### Problem

Today, `veiller-miniapp dev` (`sdk/miniapp-cli/src/dev.ts`) runs `bun run --hot server.ts`. Bun's `--hot` reloads server modules in the user's Bun process, but the WebView running on the phone has no reload trigger when the developer changes a source file. The author has to:
...
- Bun's `Bun.serve` HMR depends on Bun's own client runtime being injected at known paths. The RN WebView setup with `injectedJavaScriptBeforeContentLoaded` injects window.Veiller *before* Bun's HMR client loads, but Bun's HMR also relies on bundling assumptions that don't hold for arbitrary template setups.
...
1. **`@veiller/miniapp-cli` (`dev.ts`)**: when starting the dev server, also start a small SSE server (or wire one into the user's `server.ts`). Reuses the existing port detection / LAN IP logic.
...
3. **Injection mechanism**: two options —
   - **A. SDK auto-injects.** `@veiller/miniapp` auto-attaches a tiny client when `window.Veiller.miniappDeveloperMode === true`. Works for any template, zero config.
   - **B. Template owns it.** The `create-veiller-miniapp` template ships with the client snippet pre-installed. Only authors using the official template benefit.
4. **Client snippet**: ~15 lines of vanilla JS that opens an EventSource to `/__veiller_dev/reload` and calls `location.reload()` on receiving a `reload` event.

### Decisions

- **Live-reload client lives in the SDK.** `@veiller/miniapp` auto-injects the reload listener when `window.Veiller.miniappDeveloperMode === true`. Production miniapps don't ship the bytes (gate is the same dev-mode flag covered in section #5). Authors get live reload transparently — no template-level opt-in.
- **Reload signal flows over the shared `__veiller_dev` WebSocket** established in section #5. The dev-server side ships `{type: "reload"}`; the SDK's auto-injected client calls `location.reload()`. One channel for reload + log forwarding (multiplexed, no second WebSocket).

### Still open

- **How does the watcher know what to watch?** Default to the project root excluding `node_modules` and `dist`. Configurable via `veiller-miniapp.config.ts` later. Recommend: ship the default; defer config until someone asks.
...
Two surfaces — both ship together. Both use `@clack/prompts` for any interactive flows (already in `create-veiller-miniapp` deps; consistent with house style — see user memory `feedback_cli_prompt_library.md`).
...
veiller-miniapp permission add               # interactive: pick type, optional description
veiller-miniapp permission add MICROPHONE    # non-interactive shorthand
veiller-miniapp permission remove MICROPHONE
veiller-miniapp hardware list
veiller-miniapp hardware add                 # interactive: pick type + level
...
3. **Metro / `adb logcat` / Xcode console** — where the phone-side React Native logs appear. Only visible to engineers who have the Veiller app source and are running it from a debug build.

### Approach

Hit two surfaces on every rejection:

**A. Phone-side `console.warn`** in `LocalMiniappRuntime.ts` — visible in Metro / native console for Veiller app developers running from source. One-line, actionable:
...
3. **`@veiller/miniapp` session error handler**: include a default `console.warn` for `PERMISSION_NOT_DECLARED` and `HARDWARE_NOT_DECLARED` errors. Authors who don't subscribe to `error` still get the message in DevTools.
4. **Example template**: `VeillerProvider` (`sdk/miniapp/src/react/VeillerProvider.tsx`) can `useEffect(() => session.on("error", console.warn), [session])` so React-template authors get it for free.

### Open questions

- **Default-warn vs opt-in?** I recommend default-warn-in-developerMode, silent in production. The mode is already known via `window.Veiller.miniappDeveloperMode`. Confirm.
...
- Calling a method requiring an undeclared permission produces:
  - A `console.warn` in Metro / native console for Veiller engineers.
...
1. **Inside `@veiller/miniapp-cli`** at a stable path: `sdk/miniapp-cli/schema/miniapp.schema.json`. Editors resolve via `$schema: "./node_modules/@veiller/miniapp-cli/schema/miniapp.schema.json"`.
2. **Auto-injected `$schema`** by `create-veiller-miniapp` and by `veiller-miniapp permission/hardware` commands when they touch `miniapp.json` — so authors get autocomplete without having to know it exists.
...
2. **CLI subcommand**: `veiller-miniapp schema print` (writes to stdout) and `veiller-miniapp schema install` (adds `$schema` to the user's `miniapp.json`). Optional, low priority.
...
- New projects scaffolded by `create-veiller-miniapp` have `$schema` set.
...
## 5. WebView console bridge to `veiller-miniapp dev` terminal

### Problem

Average miniapp developers install the Veiller app from the App Store / Play Store. They don't have:
...
So when a third-party developer's miniapp throws an error in the WebView, or when the SDK emits a `console.warn` (e.g. the `PERMISSION_NOT_DECLARED` from #3), the developer has no way to see it. The Veiller app silently swallows it.

The only realistic surface a third-party developer is watching is the terminal where they're running `veiller-miniapp dev`. The webview's console messages should appear there.
...
`console.log`, `console.warn`, `console.error` calls inside the dev miniapp's WebView appear in the `veiller-miniapp dev` terminal output, prefixed with the package name, alongside the dev server's own logs.

Limited to **dev miniapps only** (`window.Veiller.miniappDeveloperMode === true`). Production miniapps must not stream console messages anywhere.
...
- **C1. Open a WebSocket from the phone to the dev server.** When the WebView loads a dev URL, the phone also connects to `ws://<dev-host>:<port>/__veiller_dev/log`. The dev server's `Bun.serve` exposes that endpoint. Phone forwards each `dev_log` over the WS. Server prints them to its stdout.
- **C2. Phone POSTs each log line to `http://<dev-host>:<port>/__veiller_dev/log`.** Stateless, no persistent connection. Higher latency per log line (HTTP overhead per message), simpler to implement.
...
4. **`sdk/miniapp-cli/src/dev.ts`**: add the `__veiller_dev/log` WebSocket endpoint to the dev server. Print incoming logs prefixed with the package name + level. Color-code (warn = yellow, error = red).
...
```
[14:32:01] [com.veiller.example] log: Subscribed to transcription
[14:32:03] [com.veiller.example] warn: Translation handler not registered yet
[14:32:05] [com.veiller.example] error: Failed to fetch /api/foo: 404
```
...
- Dev miniapp `console.log`/`warn`/`error` appears in the developer's `veiller-miniapp dev` terminal, prefixed with package name and timestamp.
...
- Killing `veiller-miniapp dev` doesn't crash the miniapp on the phone.
...
When a developer opens their miniapp in a regular laptop browser (Chrome, Safari) outside the Veiller WebView, `@veiller/miniapp`'s transport auto-detection (`sdk/miniapp/src/transport/auto.ts`) falls back to `LocalSocketTransport`, which tries to connect to `ws://127.0.0.1:8765`. That endpoint only exists on the *phone* (the `MiniSockets` server in `mobile/src/services/MiniSockets.ts`), so on the laptop it never opens.
...
Add a third transport, `MockTransport`, to `@veiller/miniapp`. Activates when:

- `window.ReactNativeWebView` is undefined (not in the Veiller WebView), AND
- The first `LocalSocketTransport` connection attempt fails within ~500ms, OR
- The author opts in explicitly via `?veiller=mock` query param / `localStorage.VEILLER_MOCK = "1"`.

Behavior:

- On `open()`: synthesize a `CONNECT_ACK` envelope with a fake `userId` (`"mock-user"`), the package name from `window.Veiller` (or `"com.mock.app"` if absent), `capabilities: null`, `visibility: "foreground"`, `colorScheme: "light"`. Deliver immediately so `session.connect()` resolves.
...
4. Document the `?veiller=mock` opt-in in the overview doc.
...
- Doesn't trigger when running inside the real Veiller WebView (the `ReactNativeWebView` check still routes to `PostMessageTransport` first).
...
4. **Live reload + WebView console bridge** (#1 + #5) — 5-7 days combined; share the `__veiller_dev` WebSocket channel from day one. Implementing in parallel is cheaper than sequentially.

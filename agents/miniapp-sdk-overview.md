...
| Package                                  | Role                                                                                         |
| ---------------------------------------- | -------------------------------------------------------------------------------------------- |
| `@veiller/miniapp`                        | Runtime SDK consumed by miniapp authors. Session, modules, transports, React hooks.          |
| `@veiller/miniapp-cli` (`veiller-miniapp`) | Author-facing CLI. `dev`, `release`, `pack`, `manifest`, `permission`, `hardware`, `schema`. |
| `create-veiller-miniapp`                  | Project scaffolder. `bunx create-veiller-miniapp my-app` → a working starter.                 |
| `sdk/example-miniapp`                    | Reference miniapp ("Live Captions" + a tester for every module). What we build against.      |

A miniapp is a **static web app** (HTML/JS/CSS bundle, any framework) with a `miniapp.json` manifest. It runs inside the Veiller app's WebView on the phone. The SDK gives it a typed API for talking to the glasses + phone.
...
**Scaling up:** for ~5+ distinct concerns, split the controller into per-concern manager classes (mirrors cloud SDK v3's user-side pattern in `Veiller-AI`, `Merge`). The example's single-class approach is fine for 1-3 concerns.
...
- `<VeillerProvider>` — root provider. Currently does one thing: keeps `<html class="dark">` in sync with the host color scheme (no FOUC because it runs during render).
...
- The CLI validates the manifest before both `dev` and `pack`. The validator mirrors `@mentra/types` string lists by hand on purpose — keeps the CLI dependency-light so `bunx veiller-miniapp` is fast.

### Host-injected globals (`window.Veiller`)

The Veiller app injects `window.Veiller` into the WebView before content loads. Authors generally use the React hooks instead, but it's there:
...
Every message is a `{payload, requestId?}` envelope, JSON over the chosen transport. `requestId` correlates request ↔ response for methods that return a value. The SDK constants (`MiniappRequestType`, `MiniappResponseType`, `MiniappStreamType`, `MiniappErrorCode`) live in `@veiller/miniapp/protocol` and are re-exported. Full enum listing in `sdk/miniapp/src/protocol.ts`.
...
**Errors.** `veiller.request` throws when the handler throws. The error is a plain `Error` with `name === "VeillerRpcError"` and `cause?.code` if the handler set one (`throw Object.assign(new Error("…"), {cause: {code: "BAD_INPUT"}})`). Distinguish by `err.name`, not `instanceof` — these errors are constructed in the WebView's bare runtime scope. Timeouts throw an `Error` with `name === "VeillerRpcTimeoutError"`; AbortSignal aborts throw a DOM-standard `AbortError`.

**Cancellation.** Pass `{signal}` to `veiller.request`. When the signal aborts, the helper sends a cancel frame; the background handler's `ctx.signal` aborts (handlers can pass it to `fetch(url, {signal})` to short-circuit a slow REST call). The caller's promise rejects with `AbortError`.
...
- **`PostMessageTransport`** — used when the miniapp runs inside the Veiller WebView. Uses `window.ReactNativeWebView.postMessage` outbound and a `window`-level `message` listener inbound.
...
```bash
bunx create-veiller-miniapp my-app   # scaffold
cd my-app
bun install
bun dev                              # starts dev server + prints QR
```

`bun dev` calls `veiller-miniapp dev` under the hood. That:
...
You scan the QR from **Veiller app → Settings → Developer settings → Mini App Development → Scan Mini App QR Code**. Phone loads your dev URL into a WebView, injects `window.Veiller`, the SDK's `PostMessageTransport` connects, you're live with hot reload.
...
bun run pack        # veiller-miniapp pack — produces the distributable ZIP
# or:
veiller-miniapp release   # build + pack + serve a QR to install on a phone
```
...
`veiller-miniapp release` is the all-in-one verb: it builds, packs, and serves the resulting ZIP behind a QR code so you can sideload it onto a phone over LAN. Pass `--no-cache` to skip the build cache.
...
```
veiller-miniapp manifest                    # interactive wizard for miniapp.json
veiller-miniapp permission list             # list declared permissions
veiller-miniapp permission add [TYPE]       # add (interactive without TYPE)
veiller-miniapp permission remove [TYPE]    # remove a declared permission
veiller-miniapp hardware list               # list hardware requirements
veiller-miniapp hardware add [TYPE] [LEVEL] # add a hardware requirement
veiller-miniapp hardware remove [TYPE]      # remove a hardware requirement
veiller-miniapp schema print                # print the miniapp.json JSON Schema
```
...
1. **`bunx create-veiller-miniapp` is the entrypoint** — but right now the template is opinionated (Bun + Bun.serve + React + Tailwind + Radix in the example). Do we ship multiple templates? Just a minimal one? A Vite variant?
...
9. **Versioning** — `@veiller/miniapp` is at `0.1.0`. Manifests get a string `version` but we have no schema-version field. We'll want one before v1 ships.
10. **`VeillerProvider` is currently 30 lines of "sync the dark class"** — fine starting surface, but a clear place to add other root-level conveniences (analytics, error boundaries, deep-link handling). Worth a quick discussion on what else belongs there.
...
- Scaffolder: `sdk/create-veiller-miniapp/`

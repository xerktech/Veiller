...
Every Veiller miniapp has two layers. Understanding the split is the key to the
...
|---|---|---|---|---|
| **Background** | Always on while the miniapp is enabled | No | Full: owns the `session` | `@veiller/miniapp/background` |
| **UI** | Only while the user has your tile open | Yes (React) | None: talks to background | `@veiller/miniapp/ui`, `@veiller/miniapp/react` |
...
time. The background bundle targets a small, cross-platform runtime. Veiller
...
Exchange only serializable data through `veiller.*` / `session.ui.*`.
...
Projects created with `create-veiller-miniapp` also check background source and
...
inside `<VeillerProvider>` and must call `veiller.ready()` on boot so the host
knows it's mounted.
...
veiller.ready(); // flush any buffered messages from background
```
...
**From the UI**, use the injected `veiller` global:

```tsx
veiller.send("ping", { at: Date.now() });            // fire-and-forget
veiller.on("pong", ({ roundtripMs }) => { /* … */ }); // subscribe
...
session.ui.handle("getNotes", async () => ({ notes }));  // respond to veiller.request
```

The two sides are intentionally asymmetric: `veiller.send` from the UI **buffers**
...
2. **UI opens**: the user taps your tile; the host spawns a WebView, injects the
   `veiller` shim, and loads `ui/index.html`. The WebView calls `veiller.ready()`,

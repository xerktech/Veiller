...
Your miniapp UI runs in a WebView inside the Veiller App, and the host draws its
...
from `@veiller/miniapp/react`, the [UI layer](/app-devs/core-concepts/webviews/react-webviews)
of the [two-layer architecture](/app-devs/core-concepts/two-layer-architecture).

```tsx
import { useSafeArea } from "@veiller/miniapp/react";
...
still works outside the Veiller App and on hosts that don't draw a menu.
</Note>
...
```tsx
import { useCapsuleHeaderStyle } from "@veiller/miniapp/react";

...
| `sdkVersion` | string | The `@veiller/miniapp` version you built against. |
| `minHostVersion` | string | Minimum Veiller App version required to run this miniapp. |
...
get autocomplete and validation as you type. `create-veiller-miniapp` adds it for
you; you can also print the schema with `veiller-miniapp schema print`.
...
- **`REQUIRED`**: glasses without this hardware can't install the miniapp (it's
  hidden in the Veiller Miniapp Store and launcher on incompatible devices).
...
```bash
veiller-miniapp manifest                  # interactive wizard
veiller-miniapp permission add MICROPHONE
veiller-miniapp hardware add DISPLAY REQUIRED
```

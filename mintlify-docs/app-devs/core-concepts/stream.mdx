...
Mentra Live. One method, `startStream`, covers both ways to stream: Veiller hosts
it for you (the default), or you publish straight to a URL you own.

```typescript src/background/index.ts
// Managed: Veiller hosts the stream and gives you playback URLs.
...
Call `startStream()` with no `direct` URL and Veiller provisions a hosted stream,
...
publish straight to it, with no Veiller relay. You manage the endpoint, and no

---
title: Quickstart
description: Scaffold a Veiller miniapp and run it on your phone in a few minutes.
---
...
### Step 1: Install the Veiller App

Install the Veiller App on your Android or iPhone from
...
```bash
bunx create-veiller-miniapp my-miniapp
```
...
`bun install` pulls in [`@veiller/miniapp`](https://www.npmjs.com/package/@veiller/miniapp)
(the runtime) and [`@veiller/miniapp-cli`](https://www.npmjs.com/package/@veiller/miniapp-cli)
(the `veiller-miniapp` tooling) automatically.
...
In the Veiller App, go to **Settings → Miniapp Developer Settings → Scan Miniapp
...
registerMiniapp((session) => {
  session.input.onButtonPress(() => {
    ...
    session.display.render([
      { type: "text", id: "hello", box: { x: 0, y: 0, w: d?.width ?? 576, h: d?.height ?? 288 }, text: "Hello from Veiller" },
    ]);
  });
...
**UI**: a normal React app that talks to the background over `veiller`. It has no
direct hardware access:

```tsx src/ui/App.tsx
export function App() {
  useEffect(() => veiller.on("pong", ({ roundtripMs }) => { /* update UI */ }), []);
  return <button onClick={() => veiller.send("ping", { at: Date.now() })}>Ping</button>;
}
```

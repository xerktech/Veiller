...
While you're building, [`veiller-miniapp dev`](/app-devs/reference/cli#dev) serves
your project over the LAN with hot reload. Scan the QR from the Veiller App and
...
serve each miniapp's runtime bundle. The Veiller App keeps a separate dev entry
...
To put a real, installed build on one or more phones without going through the Veiller Miniapp Store, run:
...
To reach users who aren't on your network, publish to the Veiller Miniapp Store.

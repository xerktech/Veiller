...
description: "Embed the full Veiller experience (miniapps, the Veiller Miniapp Store, and the developer ecosystem) inside your own phone app, driving your own smart glasses."
---

## What it is

The **Veiller OEM Integration Engine** is all of Veiller's phone-side logic packaged as a library you embed in your **own** app. Your users stay in your app and on your brand, but they get the full Veiller experience: the Veiller Miniapp Store, installable miniapps, and the same developer ecosystem that powers the Veiller App.

The engine owns everything between your glasses and Veiller's backend services:

- **Veiller Miniapp Store + runtime**: discover, install, and run miniapps on the phone.
- **Glasses I/O**: display rendering, microphone/audio routing, and the rest of the glasses feature set.
- **Backend connectivity**: talking to Veiller's backend services for the store, transcription, translation, streaming, and photos.
...
flowchart TB
  subgraph App["Your App: your branding, your user experience, your customer relationship"]
    direction TB
    subgraph Engine["Veiller OEM Integration Engine Library"]
      direction TB
      F1["Miniapp Runtime"]
      F2["Veiller Miniapp Store"]
...
- **Optional proxy**: if you route Veiller's backend services through your own infrastructure, you pass your proxy here (see below).

### 3. Connect to the backend services

The engine talks to two tiers of Veiller backend services:

- **Veiller Runtime Services.** The capabilities the engine relies on: speech-to-text, text-to-speech, translation, live streaming, and photo handling. Self-hostable and/or proxyable (run your own, route through your proxy, or both).
- **Veiller Cloud Core Services.** The Veiller Miniapp Store, the developer console, and the OEM APIs. Proxyable only (always Veiller-hosted, optionally reached through your proxy).
...
**Identity.** Your users sign in through *your* identity system. They never create a Veiller account. The engine exchanges your app's signed-in identity for a Veiller-scoped session, so your users see only your login.
...
Your adapter lives **in your app**: your Bluetooth protocol, your firmware, your transport stay private to you. You never submit glasses code to Veiller, and you never wait on a Veiller release to ship a hardware change.

## Why this works

- **Your hardware, no fork.** The Glasses Adapter is your private extension point. Add or change a model entirely within your app.
- **Your app, your brand.** Users never leave your app or see a Veiller account. The engine is a library inside your experience, not a separate app.
- **One shared ecosystem.** Every OEM's users and developers live in the same Veiller ecosystem. That's what makes the Veiller Miniapp Store worth being part of.
- **You decide what you host.** Use Veiller's backend services as-is or route them all through your own proxy.

## Availability

The Veiller OEM Integration Engine is planned for release by the **end of 2026**. To start an integration, reach out to [help@mentra.glass](mailto:help@mentra.glass).

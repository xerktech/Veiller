---
title: "Veiller OEM Integration Engine API"
description: "The engine.* API an OEM host calls to drive Veiller on its glasses — connection, pairing, device + user settings, wifi, speech, display, cloud session, miniapps, permissions, notifications, and reports, over a device-agnostic runtime."
---

<Note>
**Status: preview (Phase 1).** The Veiller OEM Integration Engine covers the full Phase-1 surface
...
The **Veiller OEM Integration Engine** is the API an OEM host app (the phone-side app
that talks to your glasses) calls to run Veiller. It ships as the `@veiller/engine` module and
...
| `gallery_mode` | boolean | `true` | capture-to-gallery enabled — when `true`, a camera-button press takes a photo to the on-device gallery. The Veiller App auto-manages this (enabled when the camera app is running or no other app is claiming button events); set it directly only if your host has no equivalent button-routing logic |
...
bridge primitives (`buildMiniappGlobalsScript`, `buildVeillerUiShim`, the VeillerJS
router), exported from `@veiller/engine` directly; your app mounts the `<WebView>` and
...
`settings`) so the first-party Veiller App can keep using them directly during migration.

<Warning>
`engine.stores.*` is a **Veiller-app convenience, not the OEM contract.** It exposes
...
<Note>
You may see internal `configureRuntime(...)` wiring in the first-party Veiller App —
that's **transitional** scaffolding for migrating Veiller's own screens, not part of

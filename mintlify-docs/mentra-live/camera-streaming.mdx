...
const stopped = await BluetoothSdk.stopVideoRecording(
  requestId,
  'https://api.example.com/veiller/video',
  'optional-token',
);
...
val stopped = sdk.stopVideoRecording(
    requestId = requestId,
    webhookUrl = "https://api.example.com/veiller/video",
    authToken = "optional-token",
)
...
cd Veiller-Bluetooth-SDK-Starter-Kit
...
import VeillerPhotoReceiver from '@veiller/bluetooth-sdk/photo-receiver';

const receiver = await VeillerPhotoReceiver.startPhotoReceiver();

const uploadSubscription = VeillerPhotoReceiver.addListener('photoUpload', (event) => {
...
try {
  ...
  await VeillerPhotoReceiver.stopPhotoReceiver();
  throw error;
}

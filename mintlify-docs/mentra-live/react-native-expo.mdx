...
description: "Use @veiller/bluetooth-sdk from a React Native or Expo development build."
icon: "mobile-screen-button"
---

React Native apps use the JavaScript package `@veiller/bluetooth-sdk`. The package contains native Android and iOS code, so it must run in a development build or production native build.

<Warning>
Expo Go cannot load `@veiller/bluetooth-sdk` because Expo Go does not include the SDK's native modules.
</Warning>
...
```bash
bun add @veiller/bluetooth-sdk@0.1.21-beta.3
...
| Hook | Use when |
| --- | --- |
| `useVeillerBluetooth` | You want one React session object for status, scan, connect/disconnect, default-device state, and gallery mode. |
...
export function DeviceScreen() {
  ...
  return (
    <View>
      ...
      <Button
        disabled={!mentra.glasses.connected || veiller.sdk.galleryMode.applying}
        title={veiller.sdk.galleryMode.enabled ? 'Disable gallery mode' : 'Enable gallery mode'}
        onPress={() => {
          void veiller.setGalleryModeEnabled(!veiller.sdk.galleryMode.enabled);
        }}
      />
...
```tsx
import {Button, Text, View} from 'react-native';
import {DeviceModels, type Device} from '@veiller/bluetooth-sdk';
import {useBluetoothScan} from '@veiller/bluetooth-sdk/react';
...
pass a `defaultDeviceStorage` adapter to `useVeillerBluetooth` if you want to
...
Use `useVeillerBluetooth()` as the React status API for connection, battery, Wi-Fi, hotspot, scan, and SDK runtime state. `setGalleryModeEnabled(...)` resolves with the ASG `SettingsAckSuccessEvent` and rejects failure statuses, `veiller.sdk.galleryMode.enabled` reflects the acknowledged gallery-mode setting, and `veiller.sdk.galleryMode.applying` is true while that ASG response is in flight.
...
# or
VEILLER_BLUETOOTH_SDK_PACKAGE_PATH=/path/to/Veiller/mobile/modules/bluetooth-sdk bunx expo run:android
```

Keep `VEILLER_BLUETOOTH_SDK_PACKAGE_PATH` in your shell or CI environment, not in committed app config.

---
title: "Android"
description: "Use the Veiller Bluetooth SDK from a native Android app."
icon: "android"
---
...
import com.veiller.bluetoothsdk.Device
import com.veiller.bluetoothsdk.DeviceModel
import com.veiller.bluetoothsdk.GlassesRuntimeState
import com.veiller.bluetoothsdk.VeillerBluetoothSdk
import com.veiller.bluetoothsdk.VeillerBluetoothSdkCallback

class GlassesController(context: Context) : VeillerBluetoothSdkCallback() {
    private val sdk = VeillerBluetoothSdk.create(
...
Use this when testing SDK source changes from a local Veiller checkout. Run the publish command from the SDK Android module with a local Gradle install, then consume the Maven-local artifact from your app:

```bash
cd /path/to/Veiller/mobile/modules/bluetooth-sdk/android
gradle :lc3Lib:publishToMavenLocal :publishToMavenLocal -PveillerPublicSdk=true
```

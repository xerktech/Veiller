...
- Confirm you are running a development build or production native build. Expo Go cannot load `@veiller/bluetooth-sdk`.
- Run `bunx expo prebuild` after adding the SDK plugin to `app.json`.
- Confirm `@veiller/bluetooth-sdk` is installed in the app that is being prebuilt.
- On iOS, rerun `bunx expo run:ios` after changing native module dependencies.
- If you are testing a local SDK package, set `VEILLER_BLUETOOTH_SDK_PACKAGE_PATH` to the same package path you installed with Bun.
...
- In React Native, render `useVeillerBluetooth()` state for connection, battery, Wi-Fi, hotspot, scan, and SDK state. In native Android/iOS apps, log the SDK status objects.
...
React Native and Expo apps use the `@veiller/bluetooth-sdk` package and must run as development builds or production native builds. Expo Go cannot load the native SDK. Start from `examples/react-native` for Expo, `examples/android` for bare Android, or `examples/ios` for bare iOS.

---
title: "API Reference"
description: "Lifecycle, status, commands, events, and shared types for the Veiller Bluetooth SDK."
icon: "book"
---

The Veiller Bluetooth SDK exposes the same core glasses lifecycle across Android, iOS, and React Native:
...
| --- | --- | --- |
| Android | `com.mentraglass:bluetooth-sdk` | `import com.veiller.bluetoothsdk.*` |
| iOS | `VeillerBluetoothSDK` Swift package | `import VeillerBluetoothSDK` |
| React Native / Expo | `@veiller/bluetooth-sdk` | `import BluetoothSdk, {DeviceModels} from '@veiller/bluetooth-sdk'` |
| React Native hooks | `@veiller/bluetooth-sdk` | `import {useVeillerBluetooth} from '@veiller/bluetooth-sdk/react'` |
| React Native photo receiver | `@veiller/bluetooth-sdk` | `import VeillerPhotoReceiver from '@veiller/bluetooth-sdk/photo-receiver'` |
...
```kotlin
val sdk = VeillerBluetoothSdk.create(
...
```swift
let sdk = VeillerBluetoothSDK(configuration: .default)
...
The SDK reports anonymous usage events to Veiller's PostHog project by default
so Veiller can understand SDK adoption and successful glasses connections:
...
configure PostHog to send these Veiller SDK usage events.
...
```xml
<meta-data
    android:name="com.veiller.bluetoothsdk.analytics.disabled"
    android:value="true" />
```
...
iOS apps can also set `VeillerBluetoothSdkAnalyticsDisabled` to `true` in
...
always sent to Veiller's PostHog project unless analytics are disabled.

Captured properties are intentionally narrow:

| Property | Meaning |
| --- | --- |
| `event_source` | Always `veiller_bluetooth_sdk`. |
...
Android exposes `GlassesRuntimeState` as `Connected` or `Disconnected`. The full state is grouped as `VeillerBluetoothState(glasses, sdk, scan)`.
...
iOS exposes `GlassesRuntimeState` as `.connected(...)` or `.disconnected(...)`. The full state is grouped as `VeillerBluetoothState(glasses, sdk, scan)`.
...
| Area | Methods |
| --- | --- |
| Status and subscriptions | `useVeillerBluetooth`, `useBluetoothScan`, and `useBluetoothEvent` for React components; `addListener` is available as the lower-level non-React subscription API |
...
React Native helper exports include `DeviceModels`, `isConnectedGlassesConnectionStatus`, `isReadyGlassesConnectionStatus`, `isBusyGlassesConnectionStatus`, `isConnectedWifiStatus`, and `isEnabledHotspotStatus`. The React subpath exports `useVeillerBluetooth`, `useBluetoothScan`, and `useBluetoothEvent`.

For React Native status UI, use `useVeillerBluetooth()` from `@veiller/bluetooth-sdk/react`. It returns `mentra.glasses`, `veiller.sdk`, and `veiller.scan` for connection, battery, Wi-Fi, hotspot, scan, and SDK runtime state.
...
```kotlin
class GlassesEvents : VeillerBluetoothSdkCallback() {
...
```swift
final class GlassesEvents: VeillerBluetoothSDKDelegate {
...
Android and iOS expose typed callbacks/delegate methods instead of the React Native string event API. Android uses `VeillerBluetoothSdkListener` methods such as `onStateChanged`, `onGlassesChanged`, `onSdkStateChanged`, `onScanChanged`, `onDeviceDiscovered`, `onButtonPress`, `onSpeakingStatus`, `onPhotoResponse`, `onPhotoStatus`, `onMicPcm`, `onStreamStatus`, and `onOtaStatus`. iOS uses `VeillerBluetoothSDKDelegate` methods such as `veillerBluetoothSDK(_:didUpdate:)`, `veillerBluetoothSDK(_:didUpdateGlasses:)`, `veillerBluetoothSDK(_:didUpdateSdkState:)`, `veillerBluetoothSDK(_:didUpdateScan:)`, `veillerBluetoothSDK(_:didDiscover:)`, `veillerBluetoothSDK(_:didReceive:)`, `veillerBluetoothSDK(_:didReceiveMicPcm:)`, and `veillerBluetoothSDK(_:didReceiveMicLc3:)`; OTA install progress arrives through `didReceive` as `.otaStartAck` or `.otaStatus`. Microphone audio callbacks use `MicPcmEvent` and `MicLc3Event` objects with the same metadata as React Native.

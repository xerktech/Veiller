/**
 * @fileoverview Integration tests for the "Veiller user" subject-token exchange.
 *
 * Covers the non-OEM subject tokens a Veiller phone presents at
 * POST /api/client/auth/exchange:
 *   - a Supabase session JWT (HS256, verified with SUPABASE_JWT_SECRET), and
 *   - a legacy veiller-core token (HS256, verified with VEILLER_CORE_JWT_SECRET).
 *
 * Both resolve to the built-in "veiller" OEM (tenantId "veiller"), which has no oems
 ...
 *
 * Run: `bun test tests/auth.veiller-user-exchange.integration.test.ts`
 */
...
// them, but keeping the whole prelude here mirrors the OEM-auth test.
{
  ...
  process.env.VEILLER_CORE_JWT_SECRET =
    "test-veiller-core-secret-not-for-production";
...
describe("Veiller user exchange — Supabase session", () => {
  test("happy path: supabase JWT → veiller tokens, tenantId 'veiller'", async () => {
    ...
    expect(claims.tenant_id).toBe("veiller");
    expect(typeof claims.sub).toBe("string");
    expect(claims.sub as string).toMatch(/^mu_/);
  });

  test("same supabase sub resolves to the same veiller user", async () => {
  ...
  test("refresh works for a veiller user (no oems record required)", async () => {
...
describe("Veiller user exchange — legacy veiller-core token", () => {
  test("happy path: veiller-core HS256 token → veiller tokens", async () => {
    const jwt = mintHs256({
      sub: "core-user-1",
      secret: process.env.VEILLER_CORE_JWT_SECRET!,
      iss: "veiller-core",
    });
    const res = await exchange(jwt);
    expect(res.status).toBe(200);
 ...
 * SUPABASE_JWT_SECRET); override `secret`/`iss` for the veiller-core or forged

...
The legacy `@veiller/cli` belongs to the old cloud and old app-server model. The
...
The existing `@veiller/miniapp-cli` has useful dev/build/pack primitives, but the
public developer entrypoint should become `@veiller/cli` with the `veiller` binary.

## Goals

- Publish a new major version of `@veiller/cli` for cloud-v2.
...
@veiller/miniapp-cli or @veiller/miniapp-tools
  reusable build, pack, manifest, schema, dev-server helpers
```

`@veiller/miniapp-cli` stays usable as a standalone CLI, but standalone usage is
unsigned. Signed release metadata and signed dev attestations are provided only
by `@veiller/cli`, which calls the miniapp package through its programmatic API
...
veiller miniapps create <packageName> --name <displayName>
veiller miniapps delete <packageName>
veiller releases list <packageName>
veiller dev
veiller build
veiller pack
veiller publish
...
present, `veiller publish` publishes that miniapp. Outside a miniapp folder, a
future explicit form can be supported:

```txt
veiller miniapp publish ./path/to/miniapp
```
...
- Calls the `@veiller/miniapp-cli` dev API with a signing callback; the SDK tool
  selects the final reachable dev URL and `@veiller/cli` signs it.
...
veiller miniapps delete com.veiller.myapp
```
...
`veiller publish` uses the local miniapp packer contract:
...
```txt
veiller login -> browser AuthKit/Console2 flow -> CLI stores credential in Keychain
```
...
mint or refresh fails. The developer should rescan a fresh `veiller dev` QR. The
system must not silently mint a token from the unsigned `com.dev` package claim.

## User Stories

1. A new developer runs `bunx @veiller/cli login` and authorizes in the browser.

...
**TL;DR:** How an OEM proves who its user is to Veiller, and how Veiller exposes that
...
trust decision. Veiller maps `(tenantId, tenantUserId)` to a `mentraUserId`. The handoff
...
and the Veiller-as-OEM bridge are in [`design.md`](./design.md).

## Problem

Cloud v2 needs an auth model that lets OEMs build their own mobile apps, keep their
own user accounts, and have those users call Veiller's backend services (audio, STT,
TTS, AI, the miniapp store) without signing in to Veiller. v1's auth assumed the
user signs in to Veiller directly.

The work splits into two related but separate decisions:

- **Q1.** How does the OEM prove who its user is, to Veiller?
- **Q2.** How does Veiller expose user identity to miniapp backends?

## Q1: OEM to Veiller auth

A user is signed in to the OEM's mobile app; the OEM's backend knows who they are.
The OEM's app needs to call Veiller's backend and have Veiller know which user is
...
signs a per-user JWT with a key registered with Veiller at onboarding; the client
exchanges it at Veiller's token endpoint (RFC 8693) for Veiller-issued Core
credentials and, for hosted Runtime, a normalized runtime credential. The SDK then
uses Veiller-issued credentials and refreshes against Veiller where applicable (the
...
| --- | --- | --- |
| SAML SSO / OIDC SSO / OAuth code | User redirected to a sign-in screen to federate identity | Rejected: assumes a Veiller user-facing UI to redirect from; the OEM owns the user-facing flow, Veiller has no screen |
| Direct-bearer JWT (LiveKit / Twilio / Agora) | OEM JWT used directly as the bearer on every call | Viable but worse: no Veiller-side revocation, OEM downtime becomes session downtime, every service reads OEM-shaped claims |
| **Token exchange (Firebase Custom Auth)** | OEM JWT exchanged once for Veiller tokens | **Chosen**: server-side revocation, refresh independence, Veiller-defined claims, standardized via RFC 8693 |
| Server-to-server registration API | OEM backend calls Veiller with an API key, no JWT crypto | Deferred: functionally equivalent outcome without OEM signing; ship one path for v2, revisit if OEMs want it |

Why token exchange: Veiller controls Core session lifetime (kill the refresh token in
our DB and the session ends), the SDK refreshes against Veiller so OEM downtime
affects only new logins, and hosted services see normalized `mentraUserId` /
`tenantId` claims with no per-request resolution.

## Q2: Veiller to miniapp identity

Today Veiller auto-auths miniapp backends with a stable user identity, which
...
trusting every OEM Veiller has approved: a compromised OEM could mint a JWT for any of
...
- `trust-all` (default): accept any verified payload. Keeps today's behavior, where
  miniapps just work.
- `veiller-direct-only`: accept only `tenantId == "veiller"`.
...
One integration flow. The OEM signs JWTs with its own private key; Veiller holds the
...
2. **OEM generates a keypair locally:**
   ...
   ```
3. **OEM registers the public key with Veiller,** two ways:
   - **Static upload:** paste `public.pem` into the portal; Veiller stores it on the
     `oems` document.
   - **JWK Set URL:** host a JWKS file at a URL and register that URL; Veiller
...
**Key rotation.** Static key: upload a new public key, Veiller replaces the stored
...
Veiller's cache refreshes on a schedule and on verification failure.
...
backend; Veiller returns Core-backed credentials. Hosted Runtime can derive or mint
...
| Claim | Meaning |
| --- | --- |
| `iss` | OEM identifier (`tenantId`). Veiller looks up the OEM's public key by this value. |
| `sub` | The OEM's own user identifier (`tenantUserId`). Stable for the user's lifetime within that OEM. |
| `aud` | Must be `"veiller"`. Audience pinning prevents a JWT meant for another service from being replayed against us. |
...
```json
{ "access_token": "<veiller-jwt>", "refresh_token": "<opaque>",
...
| `server_error` | Veiller-side issue (DB unavailable, key fetch failed) | 500 |

For the Veiller-as-OEM subject-token types (core token, Supabase session) on this
...
Revoke a single session early. OEM admin (own sessions only) or Veiller internal
...
incident). Veiller internal admin only. Sets `oems.disabled = true` (which alone
...
```js
{
  ...
  tenantId: "acme-oem",
  tenantUserId: "user-42",     // for Veiller-direct users this is the Supabase `sub`
  createdAt: ISODate
}
```
...
**Core access token (returned).** JWT signed by Veiller's access-token key, used for
Core-owned APIs:
...
Core services verify the signature against Veiller's public key, check
...
**Veiller-issued refresh token (returned).** Not a JWT: an opaque random string (256
bits, base64url). Veiller stores a **hash** (bcrypt or argon2) in `refreshTokens`
...
4. Validate claims: `aud === "veiller"`, `exp` in the future, `iat` within 5 min
...
**Revoke a single session.** Authorize (OEM admin own-only, Veiller admin any); look
...
old-key JWTs fail immediately. JWKS URL: Veiller fetches and caches with a short TTL
...
Per the Q2 decision, the auto-auth payload Veiller sends to miniapp backends carries
...
| `veiller-direct-only` | Reject if `tenantId !== "veiller"`. |
...
perspective, Veiller emits the payload with `tenantId` populated; downstream policy
...
- **Audience validation.** Reject any JWT whose `aud` is not `"veiller"`.
...
- **OEM private key never leaves the OEM's backend;** Veiller holds only the public
  key.
...
- Registers with Veiller at startup via `POST /api/oem/jwks`.
...
// 2. Exchange it with Veiller
const tokens = await fetch(`${VEILLER_URL}/api/client/auth/exchange`, {
  method: "POST",
...
// 3. Use tokens.access_token to call Veiller APIs as test-user-1
```
...
- **Migration of existing email-based Veiller users:** separate spec (see the
...
cloud-v2/packages/core/   (auth routes + services)
  ...
  services:  JWT verify (OEM-signed), JWT issue (Veiller), key resolve

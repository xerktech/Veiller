# 019 - Veiller account auth: mobile login on Cloud V2
...
**Make it possible to build a version of the Veiller mobile app with zero
...
The Veiller mobile app's login is a Cloud V1 era stack. The app embeds
...
"core token" (`VEILLER_CORE_JWT_SECRET`). Cloud V2's token exchange accepts both
...
`veiller` tenant, while real OEMs use the asymmetric JWKS path.
...
external system that calls `/api/oem/*`). For Veiller's consumer app, that role
is currently played by Supabase plus legacy Cloud V1. This issue brings
"Veiller's own OEM backend" into Cloud V2.
...
1. **Symmetric shared secrets: any backend can forge any user.** Core verifies
   ...
   `VEILLER_CORE_JWT_SECRET`, both HS256. Symmetric verification means the verify
   ...
   fix exactly this; Veiller's own login is the last thing not using it.
...
- The OEM model can still be dogfooded from inside core: the account module
  signs real Ed25519 subject tokens under a `veiller` issuer and pushes them
...
- Token/claims design: what the account module issues, how it feeds the
  existing exchange (`veiller` issuer Ed25519 subject token through the OEM
...
- OAuth redirect flow end to end (mobile deep link scheme, state/PKCE,
  the `x-veiller-public-origin` pattern the consoles use).

...
- New per-env keypair `VEILLER_ACCOUNT_JWT_PRIVATE_KEY` /
  `VEILLER_ACCOUNT_JWT_PUBLIC_KEY` (Ed25519, kid `veiller-account-1`), added to
  each environment's Doppler config and served in the well-known JWKS.
- A startup migration inserts the `oems` row for tenantId `veiller`
...
- `account.service.ts` flow for any successful verification:
  ...
  2. mint a 60s Ed25519 subject token (iss `veiller`, sub = Supabase user id,
     jti, exp),
  3. call the existing `createSession({subjectToken})` so jti replay
     protection, findOrCreateUser (tenantId `veiller`, tenantUserId = Supabase
...
- Cleanup in the same PR (enabled by no-migration, spike decision 4): delete
  ...
  `VEILLER_OEM_ID` refresh special case (the new oems row covers it).
...
app                core                        Supabase/provider
 ...
 |                  |-- 302 --> com.veiller://auth/callback?code=OTC&state
 |<-- deep link opens app
...
- The public callback URL is built from `x-veiller-public-origin` when proxied
...
- Browser: Android Custom Tabs / iOS ASWebAuthenticationSession via
  `expo-web-browser` (already an Expo app); scheme `com.veiller` is registered.
...
- **No public-origin env var is needed.** `publicOrigin()` derives the origin
  from the request (PR #3401): the `x-veiller-public-origin` header when
...
backend." Veiller must not be a privileged tenant: everything the module does
...
| holds `VEILLER_ACCOUNT_JWT_PRIVATE_KEY` | holds their own signing key |
| `veiller` oems row (static public key, seeded by migration) | their oems row, registered with Veiller |
| signs subject JWT `iss/aud = veiller`, sub, jti, iat, exp | signs the identical shape with `iss = <their tenantId>` |
...
- "OEM parity: the veiller subject token works through the PUBLIC exchange
  endpoint" - if Veiller ever grows a private path the public endpoint cannot
  serve, this fails.
- "OEM parity: a DISABLED veiller oems row blocks refresh like any OEM" -
  refresh authorization goes through the same oems-row check as every OEM (the
  old `veiller` early-return is now only a transitional fallback for
  environments whose seed migration has not run, and dies at the V1 cutover).

The `/api/account/*` endpoints themselves are Veiller's OEM-backend surface
...
the client knows Veiller's backend is special.

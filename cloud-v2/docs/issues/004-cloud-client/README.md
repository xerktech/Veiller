...
`@veiller/cloud-client` is the phone's connection to Cloud V2: a TypeScript library
(just code, no UI) that opens the connection, sends requests up, and receives events
back. The on-device Veiller Runtime (`@veiller/engine`) plugs into it; OEM hosts embed
...
- [`architecture.md`](./architecture.md): the whole picture, how a miniapp runs on
  ...
  for Veiller and OEMs, and the decisions. **Start here.**
...
- **The same code runs on the phone and on a server.** The library has no
  ...
  you import (`@veiller/cloud-client/react-native` on the phone,
  `@veiller/cloud-client/node` for tests). That's the payoff: the backend test harness
...
- **The cloud's types are the only types.** Subscriptions and events are built from
  the shared definitions in `@veiller/cloud-runtime/protocol`, the exact ones the
...
- **On device:** `@veiller/engine`, wired in at the host's `configureRuntime` hook.

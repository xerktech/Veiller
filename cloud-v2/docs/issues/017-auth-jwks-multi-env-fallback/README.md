# 017 - @veiller/auth multi-environment JWKS fallback

**Status:** Implemented, verified, and shipped to production Merge. Ordered multi-environment fallback lives in `packages/auth/src/index.ts`; 6 new tests cover the collision, first-env hit, claim-failure-not-retried, no-key, and override cases (15/15 pass). Auth build and root `tsc -b` are clean. All four live endpoints (prod, staging, dev, debug) serve JWKS (HTTP 200). Proven against a real production miniapp (Local Merge) on a real device, see "Production verification" below. Remaining: review and the eventual npm publish of `@veiller/auth`.

## Problem

`@veiller/auth` (the package a developer drops into their miniapp backend to verify the per-miniapp tokens Cloud Core mints) resolves a single JWKS URL, hard-coded to production:

```ts
const DEFAULT_JWKS_URL = "https://core.mentraglass.com/.well-known/jwks.json";
```

A developer can override `jwksUrl`, but otherwise everything verifies against prod's public keys only. That breaks anyone whose miniapp receives tokens from a non-prod Veiller backend (dev, staging, debug), and the failure mode is subtle.
...
|---|---|---|---|---|
| `veiller-miniapp-1` | key A | key B | key B | key B |
| `veiller-access-1` / `cloud-core-runtime-1` | key A | key B | key C | key C |

Because the `kid` matches across environments but the key differs, a token minted by dev (kid `veiller-miniapp-1`) is **found** in prod's JWKS by `kid`, then **fails signature verification** against prod's key. So `jose` throws a signature error, not a "no matching key" error, and the developer just sees auth failing for no obvious reason. The only current workaround is manually setting `jwksUrl`.
...
The package should work against any Veiller backend with zero configuration: try the known environment JWKS endpoints in order (prod, staging, dev, debug) and accept the first that verifies. An explicit override still wins for self-hosted or local Core.

## Required behavior

- Ship an **ordered list** of Veiller JWKS URLs and verify against each in order; first success wins.
...
- **Preserve the override.** An explicit single URL (`jwksUrl` option or `VEILLER_AUTH_JWKS_URL` env) skips the fallback entirely. This is how self-hosters and local Core keep working.
...
3. env `VEILLER_AUTH_JWKS_URLS` (comma-separated, ordered).
4. env `VEILLER_AUTH_JWKS_URL` (single).
5. Default: the built-in Veiller environment list (prod, staging, dev, debug).
...
then delivers it to the miniapp backend. `veiller dev` alone cannot mint one (it
has no user identity); a real device session pointed at the environment is what
produces it.

`scripts/verify-token-env.ts` takes such a token, fetches every Veiller
...
`createVeillerAuth().verifyToken()` path.

```
bun run packages/auth/scripts/verify-token-env.ts <token>
# or: VEILLER_TOKEN=<token> bun run packages/auth/scripts/verify-token-env.ts
```
...
- Live: against the four real endpoints, a token not signed by any Veiller
...
Symptom: the Local Merge miniapp (`com.veiller.local-merge`) showed "AI offline
...
```
[com.veiller.local-merge] console.log: 'LocalMerge: insight backend failed: backend 401'
```
...
- The Merge backend verifies with `@veiller/auth`, and its prod Doppler config had
  `VEILLER_AUTH_JWKS_URL` pinned to a single environment. A single-URL env value
...
1. Deployed the new `@veiller/auth` to `merge3` via the `merge-prod` Porter
   workflow (commit `4263725e5`). The Merge Dockerfile builds `@veiller/auth` from
   source (`COPY cloud-v2/packages/auth` then `bun run build`), so no npm publish
   was needed. Merge's backend code already calls `createVeillerAuth({ packageName })`
   with no `jwksUrl`, so the new default fallback list took effect automatically.
2. Removed `VEILLER_AUTH_JWKS_URL` from the `local-merge` `prd` and `dev` Doppler
   configs so the single-URL override no longer suppresses the fallback.
   (`VEILLER_AUTH_ISSUERS=cloud-core,veiller` was left as is.)

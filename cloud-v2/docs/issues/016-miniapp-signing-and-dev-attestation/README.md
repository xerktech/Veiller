...
1. Make `@veiller/cli` the public front door for `veiller dev`, `veiller pack`, and
   `veiller publish`.
2. Reuse the existing SDK-side `veiller-miniapp` dev/build/pack implementation
...
1. A developer runs `veiller dev` from a miniapp folder. The CLI starts the
...
4. A developer runs `veiller publish`; Core verifies the signed bundle metadata
...
`veiller publish` signs deterministic metadata:
...
`veiller dev` signs a short-lived note after the dev URL is known:
...
`@veiller/cli` is the trusted developer front door. It owns login credentials,
developer signing keys, release signatures, and dev attestations.

`@veiller/miniapp-cli` remains a useful lower-level SDK tool for local dev,
...
When a developer runs `veiller dev`, `@veiller/cli` registers or loads the local
...
`veiller-miniapp dev` still works, but it produces an unsigned QR, so miniapp
...
`VEILLER_CLI_TOKEN` is an explicit automation/E2E override and should take
...
fresh `veiller dev` QR. The SDK must surface that as auth unavailable rather than
...
| --- | --- |
| Not logged in during `veiller dev` | Miniapp runs; backend auto-auth unavailable |
...
| Attestation expires mid-use | Next token mint/refresh fails; developer rescans fresh `veiller dev` QR |
...
- E2E: `veiller dev` QR opens on phone, `session.auth.getToken()` works for the

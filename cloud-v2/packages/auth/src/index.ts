...
/**
 * Public JWKS endpoints for every Veiller-hosted environment, in fallback order.
 ...
 */
export const VEILLER_JWKS_URLS = [
...
export interface VerifiedVeillerAuth {
  mentraUserId: string;
  tenantId?: string;
  packageName: string;
  tokenId?: string;
...
export interface VeillerAuthOptions {
  /**
  ...
  packageName?: string;
  /**
   * Explicit single JWKS URL. Overrides the built-in Veiller environment list and
   ...
   * built-in Veiller environment list. Use for self-hosted multi-environment setups
   * or tests. Takes precedence over `jwksUrl`. Defaults to {@link VEILLER_JWKS_URLS}.
   */
  jwksUrls?: string[];
  /**
...
export interface VeillerAuthVariables {
  mentraAuth: VerifiedVeillerAuth;
}

export interface VeillerHonoOptions {
  /**
   ...
   */
  onUnauthorized?: (error: VeillerAuthError, c: HonoLikeContext) => Response | Promise<Response>;
}
...
export class VeillerAuthError extends Error {
...
export class VeillerAuth {
  ...
  constructor(options: VeillerAuthOptions = {}) {
  ...
  async verifyToken(token: string): Promise<VerifiedVeillerAuth> {
    const payload = await this.verifyAcrossEnvironments(token);

    const subject = stringClaim(payload.sub);
    if (!subject) {
      throw new VeillerAuthError("miniapp token missing subject");
    }
  ...
  async verifyAuthHeader(header: string | undefined | null): Promise<VerifiedVeillerAuth> {
    return this.verifyToken(extractBearerToken(header));
  }

  async verifyRequest(request: Request): Promise<VerifiedVeillerAuth> {
    return this.verifyAuthHeader(request.headers.get("Authorization"));
  }

  hono(options: VeillerHonoOptions = {}): HonoLikeMiddleware {
   ...
   * (the token was signed by a different Veiller environment that happens to use
  ...
  private async verifyAcrossEnvironments(token: string): Promise<jose.JWTPayload> {
    ...
    for (let i = 0; i < this.jwksSets.length; i++) {
      ...
      try {
        ...
        throw new VeillerAuthError(`miniapp token rejected: ${(err as Error).message}`);
      }
    }

    // Only reachable if the URL list is empty; the loop otherwise returns or throws.
    throw new VeillerAuthError(
      `miniapp token rejected: ${lastKeyMismatch?.message ?? "no JWKS endpoint configured"}`,
    );
  }
}

export function createVeillerAuth(options: VeillerAuthOptions = {}): VeillerAuth {
  return new VeillerAuth(options);
}

export async function verifyVeillerToken(
  token: string,
  options: VeillerAuthOptions = {},
): Promise<VerifiedVeillerAuth> {
  return createVeillerAuth(options).verifyToken(token);
}

export async function verifyVeillerAuthHeader(
  header: string | undefined | null,
  options: VeillerAuthOptions = {},
): Promise<VerifiedVeillerAuth> {
  return createVeillerAuth(options).verifyAuthHeader(header);
}

export function extractBearerToken(header: string | undefined | null): string {
  const match = /^Bearer\s+(.+)$/i.exec(header ?? "");
  if (!match?.[1]) {
    throw new VeillerAuthError("missing bearer token");
  }
  return match[1];
}

type JwksUrlOptions = Pick<VeillerAuthOptions, "jwksUrl" | "jwksUrls">;

/**
 * The first JWKS URL that would be tried for these options (the production
 * Veiller endpoint by default). Provided for back-compat and diagnostics; the
 * verifier itself tries every URL in {@link veillerJwksUrls}.
 */
export function veillerJwksUrl(options: JwksUrlOptions = {}): string {
  return resolveJwksUrls(options)[0];
}

/**
 * The full ordered list of JWKS URLs that verification will try for these
 * options. Defaults to {@link VEILLER_JWKS_URLS} (all Veiller environments).
 */
export function veillerJwksUrls(options: JwksUrlOptions = {}): string[] {
...
function resolvePackageName(packageName?: string): string {
  const value =
    packageName ??
    env("VEILLER_PACKAGE_NAME") ??
    env("MINIAPP_PACKAGE_NAME") ??
    env("PACKAGE_NAME");
  if (!value) {
    throw new VeillerAuthError("packageName is required");
  }
  return value;
}
...
function resolveJwksUrls(options: JwksUrlOptions): string[] {
  ...
  const envList = env("VEILLER_AUTH_JWKS_URLS")
    ?.split(",")
  ...
  const single = env("VEILLER_AUTH_JWKS_URL");
  if (single) {
    return [single];
  }

  return [...VEILLER_JWKS_URLS];
}

/**
 * True when a verification failure means the token was minted by a different
 * Veiller environment (matching `kid` resolves to the wrong key, or no key
...
function resolveIssuer(issuer?: string | string[]): string | string[] {
  ...
  const issuers = env("VEILLER_AUTH_ISSUERS")
    ?.split(",")
    .map((value) => value.trim())
    .filter(Boolean);
  if (issuers && issuers.length > 0) return issuers;

  return env("VEILLER_AUTH_ISSUER") ?? DEFAULT_ISSUER;
}

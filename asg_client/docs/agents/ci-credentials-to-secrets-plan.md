...
**Goal:** Stop hand-copying `~/.veiller/credentials/` to every CI runner. Put all *live* signing/upload credentials in GitHub Actions secrets, so any runner (mac or Ubuntu) can release-sign, and PR builds sign with the real release key. **Never modify `~/.veiller/credentials/`** — it stays as the local-dev source of truth.

## Design decisions (settled)
- **Materialize into the repo-local fallback path**, not `~/.veiller`. Both gradle files already check `~/.veiller/credentials/<file>` first, then fall back to a repo-local `credentials/` dir. In CI we write the decoded keystore to the repo-local path (gitignored, only exists in the ephemeral checkout). **Zero `build.gradle` changes.** Never touches `~/.veiller`.
...
**DROP (stale — no code references; old `com.augmentos` app, we're `com.veiller.veiller` only):**
...
- `ASG_KEYSTORE_B64`        ← `base64 -i ~/.veiller/credentials/asg-keystore.jks`
- `UPLOAD_KEYSTORE_B64`     ← `base64 -i ~/.veiller/credentials/upload-keystore.jks`
- `OTA_KEYSTORE_B64`        ← `base64 -i ~/.veiller/credentials/ota-keystore.jks`
- `ASC_API_KEY_P8_B64`      ← `base64 -i ~/.veiller/credentials/AuthKey_VKL3ALRUBR.p8`

Plain string secrets:
- `ASG_STORE_PASSWORD`, `ASG_KEY_PASSWORD`, `ASG_KEY_ALIAS` (= `asg`)
- `VEILLER_UPLOAD_STORE_PASSWORD`, `VEILLER_UPLOAD_KEY_PASSWORD`, `VEILLER_UPLOAD_KEY_ALIAS` (= `upload`)
...
### Step 1 — PR ASG build (prove it) — `veiller-asg-client-build.yml`
...
### Step 2 — PR Android build — `veiller-app-android-build.yml` (now Ubuntu)
...
Small `release-ios.mjs` edit: `loadASCConfig()` honors `VEILLER_ASC_CONFIG_PATH` env override before falling back to `~/.veiller/...`. inject-signing(ios) writes the env file + p8 to temp and sets that var. Match certs already come from secrets — unchanged.

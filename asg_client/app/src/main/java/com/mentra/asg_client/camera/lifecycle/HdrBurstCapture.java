...
public final class HdrBurstCapture {
    ...
    public interface Callback {
        ...
        default void onFinalCaptureStarted(long sensorTimestampNs) {}
    }
    ...
    public void start(
        ...
        for (int ev : HdrBurstBuilder.HDR_EV_BRACKETS) {
            ...
            HdrBurstBuilder.configureBracket(
                    StillCaptureBuilder.wrap(builder),
                    ev,
                    selectedFpsRange,
                    hasAutoFocus,
                    jpegQuality,
        ...
        session.captureBurst(
                ...
                new CameraCaptureSession.CaptureCallback() {
                    ...
                    public void onCaptureStarted(
                            @NonNull CameraCaptureSession startedSession,
                            @NonNull CaptureRequest request,
                            long timestamp,
                            long frameNumber) {
                        startedCount++;
                        if (startedCount == HdrBurstBuilder.HDR_BURST_COUNT) {
                            callback.onFinalCaptureStarted(timestamp);
                        }
                    }

                    @Override
                    public void onCaptureCompleted(
                            @NonNull CameraCaptureSession completedSession,
                            @NonNull CaptureRequest request,
                        ...
                        Log.i(
                                ...
                                "HDR: Frame "
                                        + completedCount
                                        + "/"
                                        + HdrBurstBuilder.HDR_BURST_COUNT
                                        + " completed (EV="
                                        + ev
                                        + " ISO="
                                        + iso
                                        + " exp="
                                        + (expNs != null ? expNs / 1_000_000.0 : "?")
                                        + "ms)");
                    ...
                    public void onCaptureFailed(
                            @NonNull CameraCaptureSession failedSession,
                            @NonNull CaptureRequest request,
    ...
    public boolean handleFrame(
            byte[] bytes, String targetPath, FrameSaver saver, Callback callback) {
        if (!active) {
            return false;
        }
        ...
        String bracketPath =
                new File(parentDir, HdrBurstBuilder.bracketFileSuffix(frameIdx) + ".jpg")
                        .getAbsolutePath();

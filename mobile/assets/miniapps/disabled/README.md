...
Veiller ships **no** miniapps inside the APK. Miniapps are installed at startup
from their repos' GitHub Releases, driven by the repo list in
`mobile/src/config/veillerMiniapps.ts` (see XERK-214). Nothing in
...
- `com.mentra.livestreamer-1.0.15.zip` — parked per XERK-206 (only the Even
  ...
  release and add an entry to `veillerMiniapps.ts`.

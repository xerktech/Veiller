...
import com.veiller.bluetoothsdk.utils.audio.PcmStreamManager
...
class BluetoothSdkModule : Module() {
    private var sdk: VeillerBluetoothSdk? = null
    private var deviceManager: DeviceManager? = null
    private val sdkListener =
            object : VeillerBluetoothSdkListener {
    ...
    private fun requireSdk(): VeillerBluetoothSdk =
            sdk
    ...
    override fun definition() = ModuleDefinition {
        ...
        OnCreate {
            ...
            sdk =
                    VeillerBluetoothSdk.create(
                            context,
                            VeillerBluetoothSdkConfig(
        ...
        SdkAsyncFunction("connectDefaultWithOptions") { options: Map<String, Any> ->
            sdk?.connectDefault(options.toVeillerConnectOptions())
        }

        SdkAsyncFunction("setDefaultDevice") { device: Map<String, Any>? ->
            sdk?.setDefaultDevice(device.toVeillerDevice())
        }
        ...
        //
        // Veiller-internal. 16-bit LE PCM chunks into a streaming AudioTrack
        ...
        AsyncFunction("setSttModelDetails") { path: String, languageCode: String ->
            val context =
                    appContext.reactContext
                            ?: appContext.currentActivity
                                    ?: throw IllegalStateException("No context available")
            com.veiller.bluetoothsdk.stt.STTTools.setSttModelDetails(context, path, languageCode)
        }
        ...
        AsyncFunction("getSttModelPath") { ->
            val context =
                    appContext.reactContext
                            ?: appContext.currentActivity
                                    ?: throw IllegalStateException("No context available")
            com.veiller.bluetoothsdk.stt.STTTools.getSttModelPath(context)
        }
        ...
        AsyncFunction("checkSttModelAvailable") { ->
            val context =
                    appContext.reactContext
                            ?: appContext.currentActivity
                                    ?: throw IllegalStateException("No context available")
            com.veiller.bluetoothsdk.stt.STTTools.checkSTTModelAvailable(context)
        }

        AsyncFunction("validateSttModel") { path: String ->
            com.veiller.bluetoothsdk.stt.STTTools.validateSTTModel(path)
        }
        ...
        AsyncFunction("extractTarBz2") Coroutine { sourcePath: String, destinationPath: String ->
            withContext(Dispatchers.IO) {
                com.veiller.bluetoothsdk.stt.STTTools.extractTarBz2(sourcePath, destinationPath)
            }
        }
        ...
        AsyncFunction("setTtsModelDetails") { path: String, languageCode: String ->
            val context =
                    appContext.reactContext
                            ?: appContext.currentActivity
                                    ?: throw IllegalStateException("No context available")
            com.veiller.core.tts.TTSTools.setTtsModelDetails(context, path, languageCode)
        }
        ...
        AsyncFunction("getTtsModelPath") { ->
            val context =
                    appContext.reactContext
                            ?: appContext.currentActivity
                                    ?: throw IllegalStateException("No context available")
            com.veiller.core.tts.TTSTools.getTtsModelPath(context)
        }
        ...
        AsyncFunction("getTtsModelLanguage") { ->
            val context =
                    appContext.reactContext
                            ?: appContext.currentActivity
                                    ?: throw IllegalStateException("No context available")
            com.veiller.core.tts.TTSTools.getTtsModelLanguage(context)
        }
        ...
        AsyncFunction("checkTtsModelAvailable") { ->
            val context =
                    appContext.reactContext
                            ?: appContext.currentActivity
                                    ?: throw IllegalStateException("No context available")
            com.veiller.core.tts.TTSTools.checkTTSModelAvailable(context)
        }

        AsyncFunction("validateTtsModel") { path: String ->
            com.veiller.core.tts.TTSTools.validateTTSModel(path)
        }
            ...
            val context =
                    appContext.reactContext
                            ?: appContext.currentActivity
                                    ?: throw IllegalStateException("No context available")
            withContext(Dispatchers.IO) {
                com.veiller.core.tts.TTSTools.generateTtsAudio(
                        context,
                        text,
                        modelPath,
                        outputPath,
...
private fun Map<String, Any>?.toVeillerDevice(): Device? {
...
private fun Map<String, Any>?.toVeillerConnectOptions(): ConnectOptions {

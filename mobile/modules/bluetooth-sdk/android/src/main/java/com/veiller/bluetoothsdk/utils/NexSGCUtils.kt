...
import veiller.ble.MentraosBle.DisplayText
import veiller.ble.MentraosBle.ClearDisplay
import veiller.ble.MentraosBle.DisconnectRequest
import veiller.ble.MentraosBle.PhoneToGlasses
import veiller.ble.MentraosBle.DisplayImage
import veiller.ble.MentraosBle.CanvasCreateComponent
import veiller.ble.MentraosBle.CanvasUpdateImage
import veiller.ble.MentraosBle.CanvasUpdateText
import veiller.ble.MentraosBle.CanvasDeleteComponent
import veiller.ble.MentraosBle.CanvasClear
import veiller.ble.MentraosBle.CanvasComponentType
import veiller.ble.MentraosBle.BatteryStateRequest
import veiller.ble.MentraosBle.MicStateConfig
import veiller.ble.MentraosBle.BrightnessConfig
import veiller.ble.MentraosBle.AutoBrightnessConfig
import veiller.ble.MentraosBle.HeadUpAngleConfig
import veiller.ble.MentraosBle.DisplayDistanceConfig
import veiller.ble.MentraosBle.DisplayHeightConfig
import veiller.ble.MentraosBle.VadEnabledConfig
...
object NexProtobufUtils {
    ...
    fun getProtobufSchemaVersion(context: Context): Int {
        return try {
            ...
            val fileDescriptor = veiller.ble.MentraosBle.getDescriptor().file
            
            Log.d(TAG, "Proto file descriptor: ${fileDescriptor.name}")
            
            // Method 1: Try to access the custom veiller_schema_version option
            try {
                ...
                try {
                    ...
                    veiller.ble.MentraosBle::class.java.declaredFields.forEach { field ->
    ...
    private fun readProtoVersionFromProject(context: Context): String? {
        val projectPaths = arrayOf(
            ...
            "${android.os.Environment.getExternalStorageDirectory()}/Veiller/mcu_client/mentraos_ble.proto"
        )
    ...
    private fun extractVersionFromProtoContent(content: String): String? {
        return try {
            val pattern = Regex("""option\s*\(\s*veiller_schema_version\s*\)\s*=\s*(\d+)\s*;""")

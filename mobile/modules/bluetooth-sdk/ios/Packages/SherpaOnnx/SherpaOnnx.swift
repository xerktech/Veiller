...
class SherpaOnnxOfflineRecongitionResult {
    ...
    var segmentCount: Int {
        Int(result.pointee.segment_count)
    }

    var segmentTimestamps: [Float] {
        _segmentTimestamps
    }
...
final class SherpaOnnxGenerationConfigC {
    ...
    init(_ swiftConfig: SherpaOnnxGenerationConfigSwift) {
        ...
        referenceAudioStorage = referenceAudio

        cConfig = referenceAudioStorage.withUnsafeBufferPointer { buffer in
...
class SherpaOnnxOfflineTtsWrapper {
    ...
    func generateWithCallbackWithArg(
        ...
        let wrapper: TtsProgressCallbackWithArg = { samples, n, _, rawArg in
...
struct AudioData {
    ...
    private class ManagedWave {
        ...
        init(_ p: UnsafePointer<SherpaOnnxMultiChannelWave>) {
            pointer = p
        }
    ...
    init(samples: [Float], channelCount: Int, sampleRate: Int) {
        storage = .owned(samples)
        self.channelCount = channelCount
        self.sampleRate = sampleRate
        samplesPerChannel = channelCount > 0 ? samples.count / channelCount : 0
    }

    init?(filename: String) {
        guard let ptr = SherpaOnnxReadWaveMultiChannel(filename) else { return nil }
        storage = .wrapped(ManagedWave(ptr))
        channelCount = Int(ptr.pointee.num_channels)
        samplesPerChannel = Int(ptr.pointee.num_samples)
        sampleRate = Int(ptr.pointee.sample_rate)
    }

    func withUnsafeBufferPointer<R>(_ body: (UnsafeBufferPointer<Float>) -> R) -> R {
        switch storage {
        case let .owned(array):
            return array.withUnsafeBufferPointer(body)
        case let .wrapped(managed):
...
struct SourceSeparationConfig {
    ...
    func withCConfig<R>(_ body: (UnsafePointer<SherpaOnnxOfflineSourceSeparationConfig>) -> R) -> R {
        ...
        cConfig.model.provider = b("provider", provider)
        cConfig.model.spleeter.vocals = b("spleeter.vocals", spleeter?.vocals)
        cConfig.model.spleeter.accompaniment = b("spleeter.accompaniment", spleeter?.accompaniment)
        cConfig.model.uvr.model = b("uvr.model", uvr?.model)
...
class SourceSeparator {
    ...
    init?(config: SourceSeparationConfig) {
        engine = config.withCConfig { SherpaOnnxCreateOfflineSourceSeparation($0) }

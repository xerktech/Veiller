...
public final class VeillerBluetoothSDK {
    ...
    private let configuration: VeillerBluetoothSDKConfiguration
    ...
    public var state: VeillerBluetoothState {
        VeillerBluetoothState(glassesStatus: glassesStatus, bluetoothStatus: bluetoothStatus)
    }
    ...
    private func stopScan(reason: ScanStopReason) {
        ...
        delegate?.veillerBluetoothSDK(self, didStopScan: reason)
    }
    ...
    public func requestWifiScan() async throws -> [WifiScanResult] {
        ...
        let task = Task { @MainActor [weak self] () async throws -> [WifiScanResult] in
            ...
            do {
                ...
                return try await pending.wait(timeoutMs: VeillerBluetoothSDK.wifiScanTimeoutMs)
            } catch {
    ...
    public func requestPhoto(_ request: PhotoRequest) async throws -> PhotoResponseEvent {
        ...
        do {
            let event = try await pending.wait(timeoutMs: VeillerBluetoothSDK.photoRequestTimeoutMs)
    ...
    private func sendNextStreamKeepAlive(for tracker: ActiveStreamKeepAlive) {
        ...
        if tracker.armed, tracker.pendingAckId != nil {
            ...
            if tracker.missedAckCount >= 3 {
                ...
                delegate?.veillerBluetoothSDK(self, didReceive: .streamStatus(event))
                stopStreamKeepAliveMonitor()
                DeviceManager.shared.stopStream()
                return
            }
    ...
    private func dispatchStoreUpdate(_ category: String, _ changes: [String: Any]) {
        ...
        case "glasses":
            ...
            let nextState = state
            delegate?.veillerBluetoothSDK(self, didUpdate: nextState)
            delegate?.veillerBluetoothSDK(self, didUpdateGlasses: nextState.glasses)
        case ObservableStore.bluetoothCategory:
            let nextState = state
            delegate?.veillerBluetoothSDK(self, didUpdate: nextState)
            delegate?.veillerBluetoothSDK(self, didUpdateSdkState: nextState.sdk)
            delegate?.veillerBluetoothSDK(self, didUpdateScan: nextState.scan)
    ...
    private func dispatchDefaultDeviceChanged() {
        delegate?.veillerBluetoothSDK(self, didChangeDefaultDevice: currentDefaultDevice())
    }
...
private func dispatchDiscoveredDevices(_ rawSearchResults: Any?) {
        ...
        for result in results {
            ...
            delegate?.veillerBluetoothSDK(self, didDiscover: device)
        }
    }
    ...
    private func dispatchBridgeEvent(_ eventName: String, _ data: [String: Any]) {
        switch eventName {
        case "log":
            delegate?.veillerBluetoothSDK(self, didLog: data["message"] as? String ?? data.description)
        case "button_press":
            ...
            delegate?.veillerBluetoothSDK(self, didReceive: .buttonPress(event))
        case "touch_event":
            delegate?.veillerBluetoothSDK(self, didReceive: .touch(TouchEvent(values: data)))
        case "mic_pcm":
            let event = MicPcmEvent(values: data)
            if !event.pcm.isEmpty {
                delegate?.veillerBluetoothSDK(self, didReceiveMicPcm: event)
            }
        case "mic_lc3":
            let event = MicLc3Event(values: data)
            if !event.lc3.isEmpty {
                delegate?.veillerBluetoothSDK(self, didReceiveMicLc3: event)
            }
        case "local_transcription":
            ...
            delegate?.veillerBluetoothSDK(self, didReceive: .localTranscription(event))
        case "voice_activity_detection_status":
            delegate?.veillerBluetoothSDK(
                self,
        ...
        case "hotspot_status_change":
            ...
            delegate?.veillerBluetoothSDK(self, didReceive: .hotspotStatus(event))
        case "wifi_status_change":
            let event = WifiStatusEvent(values: data)
            handleWifiStatusForRequests(event)
            delegate?.veillerBluetoothSDK(self, didReceive: .wifiStatus(event))
        case "wifi_scan_result":
            ...
            delegate?.veillerBluetoothSDK(self, didReceive: .raw(name: "wifi_scan_result", values: data))
        case "hotspot_error":
            let event = HotspotErrorEvent(values: data)
            handleHotspotErrorForRequests(event)
            delegate?.veillerBluetoothSDK(self, didReceive: .hotspotError(event))
        case "gallery_status":
            let event = GalleryStatusEvent(values: data)
            pendingGalleryStatus?.resolve(event)
            delegate?.veillerBluetoothSDK(self, didReceive: .raw(name: "gallery_status", values: event.values))
        case "photo_response":
            let event = PhotoResponseEvent(values: data)
            handlePhotoResponseForRequests(event)
            delegate?.veillerBluetoothSDK(self, didReceive: .photoResponse(event))
        case "photo_status":
            delegate?.veillerBluetoothSDK(self, didReceive: .photoStatus(PhotoStatusEvent(values: data)))
        case "camera_status":
            let event = CameraStatusEvent(values: data)
            handleCameraStatusForRequests(event)
            delegate?.veillerBluetoothSDK(self, didReceive: .cameraStatus(event))
        case "video_recording_status":
            let event = VideoRecordingStatusEvent(values: data)
            handleVideoRecordingStatusForRequests(event)
            delegate?.veillerBluetoothSDK(self, didReceive: .videoRecordingStatus(event))
        case "media_success", "media_error":
            let event = MediaUploadEvent(values: data)
            handleMediaUploadForRequests(event)
            delegate?.veillerBluetoothSDK(self, didReceive: .mediaUpload(event))
        case "rgb_led_control_response":
            let event = RgbLedControlResponseEvent(values: data)
            handleRgbLedResponseForRequests(event)
            delegate?.veillerBluetoothSDK(self, didReceive: .rgbLedControlResponse(event))
        case "stream_status":
        ...
        case "keep_alive_ack":
            ...
            if !handleStreamKeepAliveAck(event) {
                delegate?.veillerBluetoothSDK(self, didReceive: .keepAliveAck(event))
            }
        case "ota_start_ack":
            var values = data
            ...
            delegate?.veillerBluetoothSDK(self, didReceive: .otaStartAck(event))
        case "ota_status":
            var resultValues = data
            resultValues["type"] = "ota_status"
            pendingOtaQuery?.resolve(OtaQueryResult(values: resultValues))
            delegate?.veillerBluetoothSDK(self, didReceive: .otaStatus(OtaStatusEvent(values: resultValues)))
        case "settings_ack":
            let event = SettingsAckEvent(values: data)
            handleSettingsAckForRequests(event)
            delegate?.veillerBluetoothSDK(self, didReceive: .settingsAck(event))
        case "version_info":
            let event = VersionInfoResult(values: data)
            pendingVersionInfo?.resolve(event)
            delegate?.veillerBluetoothSDK(self, didReceive: .versionInfo(event))
        case "compatible_glasses_search_stop":
            delegate?.veillerBluetoothSDK(self, didStopScan: .completed)
        case "pair_failure":
            delegate?.veillerBluetoothSDK(
                self,
        ...
        default:
            delegate?.veillerBluetoothSDK(self, didReceive: .raw(name: eventName, values: data))
        }
    }
}

...
Veiller Engine — the on-device miniapp library.

## Installation

```sh
npm install @veiller/engine
```

> **Peer packages:** the engine's `@veiller/*` peer dependencies (`crust`,
...
> `@veiller/engine` from plain Node (`require`/ESM) is not supported at 0.1.x —
...
- **`@veiller/engine`** (main, `src/index.ts`) — the OEM-facing surface: the
...
- **`@veiller/engine/internal`** (`src/internal.ts`) — the migration-era
...
- **`@veiller/engine/devtools`** (`src/devtools.ts`) — debug-only singletons
...
```ts
import {engine, decideDevLaunchRoute} from "@veiller/engine"
import {webviewBridge, buildMiniappGlobalsScript} from "@veiller/engine/internal"
...
- `buildMiniappGlobalsScript` — builds the `window.Veiller` injection script

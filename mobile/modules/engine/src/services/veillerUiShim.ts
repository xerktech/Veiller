/**
 * window.veiller — WebView-side bridge to the per-miniapp background JSContext.
 *
 ...
 *
 * The shim:
 *   - Buffers `veiller.send(channel, payload)` calls until `veiller.ready()`
 ...
 *   - Installs `window.__veiller.recv(envelope)` for the host to push
 ...
 *
 * This file exports a `buildVeillerUiShim()` function returning the JS
 * source as a string. The host's VeillerUIRouter wraps it inside the
...
export interface VeillerUiShimOptions {
  packageName: string
  /** @deprecated No longer used — heartbeat removed since WebViews are foreground-only. */
  heartbeatIntervalMs?: number
}

export function buildVeillerUiShim(options: VeillerUiShimOptions): string {
  const packageNameJson = JSON.stringify(options.packageName)
  // The IIFE assembles a typed `veiller` global plus an internal
  // `__veiller` for host inbound calls. Quoted strings are JSON-safe so
  // direct injection inside another JS literal also works.
  return `
(function () {
  if (typeof window === 'undefined') return;
  if (window.veiller && window.__veiller) return;
  ...
  var openHandlers = [];
  var closeHandlers = [];
  // Queued veiller.send calls before veiller.ready() acks. FIFO drained
  // on ack. Buffered so user input (button taps, etc.) isn't dropped
  // when the WebView mounts faster than the host expects.
  var outboundQueue = [];
  // Inbound payloads that arrived before any veiller.on subscriber was
  ...
  // veiller.on(channel, ...) subscriber, then discarded. Subsequent
  ...
  function VeillerRpcError(message, code, stage, transport) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      function settle(envelope) {
        ...
        if (envelope && envelope.ok === true) {
          ...
          reject(VeillerRpcError(envelope.error && envelope.error.message || 'RPC failed', envelope.error && envelope.error.code, envelope.error && envelope.error.stage, envelope.error && envelope.error.transport));
        } else {
  ...
  function fireChannel(channel, payload) {
    ...
    if (!list || list.length === 0) {
      // No subscriber yet — stash for the first veiller.on(channel, ...).

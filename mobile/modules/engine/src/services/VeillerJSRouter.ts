/**
 * @fileoverview VeillerJSRouter — RN-side host that bridges the per-miniapp
 ...
 * consumes Crust's `veillerjs_message` Expo event and forwards its
 ...
 *     [rawJsonString])` → `veillerjs_message` event → router →
 ...
 *     `Crust.veillerJsDispatchToJs(packageName, {kind: "bridge", raw})`.
 *     The polyfill's `__deliver` recognises `kind === "bridge"` and calls
 *     `__veillerDeliverBridgeRaw(raw)`, which routes into the SDK's
...
import type {VeillerJSCrashController} from "./VeillerJSCrashController"
import {VeillerJSLogRingBuffer, VeillerJSLogThrottle, redactSecrets} from "./VeillerJSLogPipeline"
import {islandNotifications} from "./NotificationsEmitter"
import type {VeillerUIRouter} from "./VeillerUIRouter"
 ...
 */
export interface VeillerJSCrustBinding {
  veillerJsDispatchToJs(packageName: string, envelope: Record<string, unknown>): Promise<void> | void
  veillerJsSetManifest(packageName: string, permissions: string[]): Promise<void> | void
  veillerJsLoadPolyfillBundle?: () => string
  veillerJsSpawn?: (packageName: string, polyfill: string, miniappJs: string) => Promise<boolean> | boolean
  veillerJsKill?: (packageName: string) => Promise<void> | void
...
const defaultLogger: RouterLogger = {
  log: (m, p) => console.log(`[VeillerJSRouter] ${m}`, p ?? ""),

  warn: (m, p) => console.warn(`[VeillerJSRouter] ${m}`, p ?? ""),

  error: (m, p) => console.error(`[VeillerJSRouter] ${m}`, p ?? ""),
}
...
export class VeillerJSRouter {
   ...
   */
  crashController: VeillerJSCrashController | null = null
   ...
   * defaults.
   */
  logThrottle: VeillerJSLogThrottle = new VeillerJSLogThrottle()
  logRing: VeillerJSLogRingBuffer = new VeillerJSLogRingBuffer()
   ...
   */
  uiRouter: VeillerUIRouter | null = null

  constructor(
    private readonly runtime: LocalMiniappRuntime,
    private readonly crust: VeillerJSCrustBinding,
    private readonly logger: RouterLogger = defaultLogger,
  ) {}

  /**
   * Subscribe to Crust's `veillerjs_message` event. Idempotent — calling
   * start() twice attaches a single listener.
   */
  start(): void {
    if (this.subscription) return
    this.subscription = this.crust.addListener("veillerjs_message", (raw) => {
      try {
   ...
   * `sendMessage` that calls `Crust.veillerJsDispatchToJs(packageName,
  ...
  async spawnAndRegister(
    ...
    if (!this.crust.veillerJsSpawn) {
      this.logger.warn("veillerJsSpawn not available — host binding missing native function")
      return false
    }
    const polyfill = this.crust.veillerJsLoadPolyfillBundle?.() ?? ""
    if (!polyfill) {
      this.logger.warn(`no polyfill bundle loaded for ${packageName}; spawn will likely fail`)
    }
    const ok = await this.crust.veillerJsSpawn(packageName, polyfill, miniappJs)
    if (!ok) {
      this.logger.error(`spawn failed for ${packageName}`)
      return false
    }
    // Two distinct permission gates:
    //   1. Native dispatcher (JSContext → native bridge): `veillerJsSetManifest`
    ...
    if (options?.permissions && options.permissions.length > 0) {
      await this.crust.veillerJsSetManifest(packageName, options.permissions)
    }
    ...
    // resolves `__veillerInitCallback(sessionId)`. The SDK's
    ...
    void this.crust.veillerJsDispatchToJs(packageName, {kind: "init", sessionId})
  ...
  private handleCrash(packageName: string, reason: string): void {
    ...
    const timer = setTimeout(() => {
      ...
      void (async () => {
        try {
          await this.crust.veillerJsKill?.(packageName)
        } catch {
          /* native may have already torn down */
        }
        const ok = await this.crust.veillerJsSpawn?.(
          packageName,
          this.crust.veillerJsLoadPolyfillBundle?.() ?? "",
          cached.miniappJs,
        )
        if (!ok) {
        ...
        if (cached.permissions.length > 0) {
          await this.crust.veillerJsSetManifest(packageName, cached.permissions)
        }
  ...
  async unregister(packageName: string): Promise<void> {
    ...
    if (this.crust.veillerJsKill) {
      try {
        await this.crust.veillerJsKill(packageName)
      } catch (e) {
        this.logger.warn(`veillerJsKill threw for ${packageName}: ${String(e)}`)
      }
    }
  }
  ...
  private handleOutbound(msg: OutboundMessagePayload): void {
    ...
    if (!packageName || !iface || !method) {
      this.logger.warn(`bad veillerjs_message — missing fields`, msg)
      return
    }
    ...
    if (iface === "__log") {
      ...
      // Forward to the `veiller-miniapp dev` sidecar (if one is connected
      // for this package). Source="background" so the CLI tags lines as
      // [VeillerJS]. The bridge silently drops when no dev server is up,
  ...
  private dispatchBridgeRaw(packageName: string, raw: string): void {
    void this.crust.veillerJsDispatchToJs(packageName, {kind: "bridge", raw})
  }
}

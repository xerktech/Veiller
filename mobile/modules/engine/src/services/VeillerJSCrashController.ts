/**
 * VeillerJSCrashController — crash recovery state machine for the
 * per-miniapp JSContexts owned by VeillerJSRouter.
 *
 *   RUNNING
 ...
 * veillerJsSpawn / veillerJsKill. Callers (VeillerJSRouter) read the
...
export class VeillerJSCrashController {
  ...
  /** Tracking starts when VeillerJSRouter spawns a fresh JSContext. */
  ...
  /** Called when VeillerJSRouter explicitly tears down (user-initiated). */

...
function buildMockCrust(): {
  ...
  const binding: VeillerJSCrustBinding = {
    ...
    addListener(event, handler) {
      ...
      } as unknown as ReturnType<VeillerJSCrustBinding["addListener"]>
    },
  }
  return {
    binding,
...
describe("VeillerJSRouter", () => {
  ...
  let router: VeillerJSRouter

  beforeEach(() => {
    crust = buildMockCrust()
    runtimeMock = buildMockRuntime()
    logger = silentLogger()
    router = new VeillerJSRouter(runtimeMock.runtime, crust.binding, logger)
  })
  ...
  test("start() attaches a single veillerjs_message listener", () => {
    router.start()
    router.start()
    crust.emit("veillerjs_message", {
      packageName: "com.foo",
      iface: "__log",
      method: "log",
      argsJson: '["hi"]',
  ...
  test("__log frames over the throttle threshold drop silently + emit a [throttled N] summary", () => {
    ...
    const {VeillerJSLogThrottle} = require("../VeillerJSLogPipeline")
    let clock = 0
    router.logThrottle = new VeillerJSLogThrottle({
      tokensPerSecond: 10,
      bucketCapacity: 3,
      now: () => clock,
    })
  ...
  test("malformed event missing packageName/iface logs a warning", () => {
    router.start()
    crust.emit("veillerjs_message", {iface: "x", method: "y"})
    expect(logger.warn).toHaveBeenCalled()
  })
  ...
  test("registerApp() registers a sendMessage that calls veillerJsDispatchToJs with kind=bridge", () => {
  ...
  test("crash controller receives __error frames and schedules respawn", async () => {
    const {VeillerJSCrashController} = await import("../VeillerJSCrashController")
    let clock = 0
    const controller = new VeillerJSCrashController({
      now: () => clock,
      backoffMs: [2_000],
      maxRetries: 3,
    })

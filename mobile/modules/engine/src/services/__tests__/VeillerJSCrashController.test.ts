...
describe("VeillerJSCrashController", () => {
  let clock: number
  let controller: VeillerJSCrashController

  beforeEach(() => {
    clock = 1_000_000
    controller = new VeillerJSCrashController({
      now: () => clock,
  ...
  describe("default zero-retry policy", () => {
    test("first crash with no opts transitions straight to CRASHLOOP_DISABLED", () => {
      const c = new VeillerJSCrashController({now: () => clock})
      c.onSpawn("com.foo")

...
import {VeillerUIRouter, type VeillerUICrustBinding} from "../VeillerUIRouter"

function buildMockCrust() {
  const dispatchCalls: Array<{packageName: string; envelope: Record<string, unknown>}> = []
  const binding: VeillerUICrustBinding = {
...
function bindCapture(router: VeillerUIRouter, packageName: string) {
...
describe("VeillerUIRouter — routeFromWebView", () => {
  let crust: ReturnType<typeof buildMockCrust>
  let router: VeillerUIRouter

  beforeEach(() => {
    crust = buildMockCrust()
    router = new VeillerUIRouter(crust.binding)
  })
...
describe("VeillerUIRouter — routeFromBackground", () => {
  let crust: ReturnType<typeof buildMockCrust>
  let router: VeillerUIRouter
  ...
  test("UI_SEND with a bound WebView injects a msg frame into window.__veiller.recv", () => {
    ...
    expect(injects[0]).toContain("window.__veiller")
...
describe("VeillerUIRouter — lifecycle", () => {
  let crust: ReturnType<typeof buildMockCrust>
  let router: VeillerUIRouter

...
describe("VeillerJSLogThrottle", () => {
  let clock: number
  let throttle: VeillerJSLogThrottle

  beforeEach(() => {
    clock = 1_000_000
    throttle = new VeillerJSLogThrottle({
...
describe("VeillerJSLogRingBuffer", () => {
  test("retains the last N lines per package", () => {
    const buf = new VeillerJSLogRingBuffer(3)
    buf.push("a", "1")
    buf.push("a", "2")
    buf.push("a", "3")
    buf.push("a", "4")

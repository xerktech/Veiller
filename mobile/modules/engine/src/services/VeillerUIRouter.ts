/**
 * VeillerUIRouter — bidirectional bus between a per-miniapp UI WebView
 * and its bound background JSContext.
 *
 * The flow:
 *
 *   WebView (veiller.send/on)                Background (session.ui.send/on)
 *      │                                       │
 ...
 *      │                                      │
 *      ▼                                      ▼
 *   MiniappHost.onMessage              VeillerJSRouter (recognises payload.type==="UI_SEND")
 *      │                                      │
 *      ▼                                      ▼
 *      ──── VeillerUIRouter.routeFromWebView   ──── VeillerUIRouter.routeFromBackground
 *           ──► Crust.veillerJsDispatchToJs        ──► webView.injectJavaScript("window.__veiller.recv(...)")
...
export class VeillerUIRouter {
  private readonly bindings: Map<string, BoundWebView> = new Map()
  private readonly crust: VeillerUICrustBinding

  constructor(crust: VeillerUICrustBinding) {
    this.crust = crust
  }
   ...
   * `window.__veiller.recv(...)` strings to deliver background-side
   * frames into the WebView's veiller global.
   *
   ...
   */
  bindWebView(packageName: string, injectFn: VeillerUIInjectFn): void {
  ...
  /**
   * Called by VeillerJSRouter when it sees a `__bridge.send` outbound
  ...
  private deliverToBackground(packageName: string, data: Record<string, unknown>): void {
    ...
    void this.crust.veillerJsDispatchToJs(packageName, {
      kind: "bridge",

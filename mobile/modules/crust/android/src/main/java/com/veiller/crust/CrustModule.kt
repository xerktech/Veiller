package com.veiller.crust

import android.content.BroadcastReceiver
import android.util.Log
import com.veiller.crust.services.NotificationListener
import com.veiller.crust.services.NotificationProcessBridge
import expo.modules.kotlin.modules.Module
import expo.modules.kotlin.modules.ModuleDefinition
import java.net.URL

import com.veiller.crust.navigation.NavigationManager
import com.veiller.crust.heading.HeadingManager
import com.veiller.crust.jsc.JSCRuntime
import com.veiller.crust.jsc.InstalledMiniappManifest
import com.veiller.crust.jsc.JSCPolyfillBridge
...
class CrustModule : Module() {
  ...
  private fun installRuntimeIfPossible(reason: String): Boolean {
    ...
    if (ctx == null) {
      Log.w(TAG, "VeillerJS: install deferred ($reason) — no context yet")
      return false
    }
    val runtime = JSCRuntime.shared(ctx)
    runtime.onOutbound = { msg ->
      sendEvent("veillerjs_message", msg.payload)
    }
    JSCPolyfillBridge.install(runtime)
    runtimeInstalled = true
    Log.i(TAG, "VeillerJS: runtime installed ($reason)")
    return true
  }
  ...
  override fun definition() = ModuleDefinition {
    ...
    Events(
      ...
      "onNavOffRoute",
      "onHeading",
      // VeillerJS — per-miniapp JSContext outbound message bus.
      // VeillerJSRouter subscribes via Crust.addListener.
      "veillerjs_message",
    )
    ...
    // MARK: - Veiller Notification Commands
    ...
    // MARK: - VeillerJS Runtime

    AsyncFunction("veillerJsSpawn") { packageName: String, polyfillBundle: String, miniappJs: String ->
      val ctx =
              appContext.reactContext
                      ?: appContext.currentActivity
                              ?: throw IllegalStateException("VeillerJS: no context")
      ...
      installRuntimeIfPossible("veillerJsSpawn")
    ...
    AsyncFunction("veillerJsEvaluate") { packageName: String, source: String ->
      val ctx =
              appContext.reactContext
                      ?: appContext.currentActivity
                              ?: throw IllegalStateException("VeillerJS: no context")
    ...
    AsyncFunction("veillerJsKill") { packageName: String ->
      val ctx =
              appContext.reactContext
                      ?: appContext.currentActivity
                              ?: throw IllegalStateException("VeillerJS: no context")
    ...
    AsyncFunction("veillerJsDispatchToJs") { packageName: String, envelope: Map<String, Any?> ->
      val ctx =
              appContext.reactContext
                      ?: appContext.currentActivity
                              ?: throw IllegalStateException("VeillerJS: no context")
    ...
    AsyncFunction("veillerJsSetManifest") { packageName: String, permissions: List<String> ->
      val ctx =
              appContext.reactContext
                      ?: appContext.currentActivity
                              ?: throw IllegalStateException("VeillerJS: no context")
    ...
    Function("veillerJsAlivePackages") {
      val ctx =
              appContext.reactContext
                      ?: appContext.currentActivity
      if (ctx == null) {
    ...
    AsyncFunction("veillerJsDebugForceGC") { packageName: String ->
      val ctx =
              appContext.reactContext
                      ?: appContext.currentActivity
    ...
    Function("veillerJsLoadPolyfillBundle") {
      val ctx =
              appContext.reactContext
                      ?: appContext.currentActivity
      if (ctx == null) {
    ...
    AsyncFunction("processGalleryImage") {
      ...
      try {
        ...
        val processingTimeMs =
                com.veiller.crust.utils.ImageProcessor.process(
                        inputPath,
                        outputPath,
                        lensCorrection,
                        colorCorrection,
    ...
    AsyncFunction("mergeHdrBrackets") {
      ...
      try {
        val processingTimeMs =
                com.veiller.crust.utils.ImageProcessor.mergeHdr(
                        underPath,
                        normalPath,
                        overPath,
                        outputPath,
    ...
    AsyncFunction("stabilizeVideo") { inputPath: String, imuPath: String, outputPath: String ->
      try {
        ...
        val processingTimeMs =
                com.veiller.crust.utils.VideoStabilizer.stabilize(inputPath, imuPath, outputPath)
   ...
   * capture time, and Veiller album path let the retry return that receipt instead of inserting a
  ...
  private fun findExistingGalleryAsset(
    ...
    if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.Q &&
      ...
      // reconcilable, while still requiring an exact Veiller album match.

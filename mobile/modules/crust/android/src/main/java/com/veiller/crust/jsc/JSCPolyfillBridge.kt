 ...
 * Promise correlator (the `__veillerSendRequest` reqId map) takes it from
 * there.
 *
...
object JSCPolyfillBridge {
    private const val TAG = "VeillerJS.PolyfillBridge"
    ...
    private fun installFetch(runtime: JSCRuntime) {
        runtime.dispatcher.register("fetch", "request") { packageName, args, reqId ->
            ...
            if (reqId == null) {
                return@register JSCDispatchOutcome.Error("INVALID_ARGS", "fetch.request requires reqId (use __veillerSendRequest)")
            }

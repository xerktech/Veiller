 ...
 * [VeillerJSRouter] is the actual handler.
 */
sealed class JSCDispatchOutcome {
    ...
    /** Forward to RN via the `veillerjs_message` Expo event. */
...
class JSCDispatcher(private val appContext: Context) {
    companion object {
        private const val TAG = "VeillerJS.Dispatcher"
    }
    ...
    private fun prefs(packageName: String): SharedPreferences {
        return appContext.getSharedPreferences("VeillerJS-$packageName", Context.MODE_PRIVATE)
    }
}

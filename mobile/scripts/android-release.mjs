...
if (nameSuffix) {
  ...
  const newPkg = `com.xerktech.veiller.${nameSuffix}`
  const baseClient = gs.client?.find(
    (c) => c.client_info?.android_client_info?.package_name === 'com.xerktech.veiller',
  )
...
if (nameSuffix) {
  console.log(`   Package: com.xerktech.veiller.${nameSuffix}`)
